...
describe("CommandPalette lifecycle", () => {
  ...
  it.each([
    ...
    "shows Ask Granted only when availability is $available",
    async ({ available, expectedCount }) => {
      const { gateway } = createGateway(true);
      const { palette } = await mountPalette(
        createContext(
      ...
      await enterQuery(palette, "granted");

      expect(findPaletteOption(palette, "Ask Granted", true) ? 1 : 0).toBe(expectedCount);
    },
  );

  it("opens Ask Granted from its palette action", async () => {
    const { gateway } = createGateway(true);
    const { palette } = await mountPalette(
      createContext(
        gateway,
    ...
    try {
      findPaletteOption(palette, "Ask Granted", true)?.click();
    } finally {
