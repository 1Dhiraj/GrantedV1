...
it("keeps explicit parents authoritative and trims the existing configured root", () => {
  ...
  vi.stubEnv("GRANTED_UI_E2E_ARTIFACT_DIR", `  ${configured}  `);
...
it.each([undefined, "", "   "])(
  ...
  (root) => {
    vi.stubEnv("GRANTED_UI_E2E_ARTIFACT_DIR", root);
...
it("does not enable optional capture when allocating an always-on scenario", () => {
  ...
  vi.stubEnv("GRANTED_UI_E2E_ARTIFACT_DIR", undefined);
  ...
  expect(process.env.GRANTED_CAPTURE_UI_PROOF).toBe("0");
  expect(process.env.GRANTED_UI_E2E_ARTIFACT_DIR).toBeUndefined();
});
