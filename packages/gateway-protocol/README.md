...
as `PROTOCOL_VERSION` from `@granted/gateway-protocol/version`. Gateway protocol
...
```bash
npm install --save-exact @granted/gateway-protocol@2026.8.1
```
...
- `@granted/gateway-protocol` exports runtime validators, selected schemas, error
  formatting, and their TypeScript types. This is the main TypeBox-backed entry.
- `@granted/gateway-protocol/schema` exports the TypeBox schema graph, including
  the `ProtocolSchemas` registry used by generators.
- `@granted/gateway-protocol/frame-guards` exports dependency-free structural
  guards for gateway event and response envelopes.
- `@granted/gateway-protocol/client-info` exports client ID, mode, and capability
  registries plus normalization helpers.
- `@granted/gateway-protocol/connect-error-details` exports structured connect
  error readers and recovery metadata.
- `@granted/gateway-protocol/gateway-error-details` exports helpers for reading
  structured details from general gateway errors.
- `@granted/gateway-protocol/startup-unavailable` exports startup retry constants
  and helpers.
- `@granted/gateway-protocol/version` exports the current and minimum accepted
  protocol versions.
...
```ts
import { formatValidationErrors, validateRequestFrame } from "@granted/gateway-protocol";
...
```ts
import { isGatewayEventFrame } from "@granted/gateway-protocol/frame-guards";
...
import { MIN_CLIENT_PROTOCOL_VERSION, PROTOCOL_VERSION } from "@granted/gateway-protocol/version";
...
Download it as a file; `@granted/gateway-protocol/protocol.schema.json` is not an
