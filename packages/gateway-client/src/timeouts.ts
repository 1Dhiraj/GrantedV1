...
export function getConnectChallengeTimeoutMsFromEnv(
  ...
  const raw = env.GRANTED_CONNECT_CHALLENGE_TIMEOUT_MS;
  if (raw) {
...
export function resolvePreauthHandshakeTimeoutMs(params?: {
  ...
  const configuredTimeout =
    env.GRANTED_HANDSHAKE_TIMEOUT_MS ||
    (isTestRuntimeEnv(env) ? env.GRANTED_TEST_HANDSHAKE_TIMEOUT_MS : undefined);
