...
function normalizeLifecycleEndEventType(data: JsonObject): GrantedEventType {
  const status = readLowerString(data.status);
  const stopReason = readLowerString(data.stopReason);
...
function normalizeAgentEventType(payload: JsonObject): GrantedEventType {
...
function normalizeNamedEventType(event: GatewayEvent): GrantedEventType {
...
export function normalizeGatewayEvent(event: GatewayEvent): GrantedEvent {
  const payload = asRecord(event.payload);
