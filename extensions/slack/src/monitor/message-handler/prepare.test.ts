...
import { resolveAgentRoute, resolveThreadSessionKeys } from "granted/plugin-sdk/routing";
import { upsertSessionEntry, type SessionEntry } from "granted/plugin-sdk/session-store-runtime";
...
Second paragraph should still reach the agent after Slack's preview cutoff.`;
  ...
  it("records skipped no-mention room images as pending history media", async () => {
    ...
    try {
      const slackCtx = createInboundSlackCtx({
        cfg: { channels: { slack: { enabled: true } } } as GrantedConfig,
        defaultRequireMention: true,
      });
...
describe("slack implicit mention policy", () => {
  ...
  async function prepareThreadMessage(params: {
    ...
    const { storePath } = storeFixture.makeTmpStorePath();
    vi.spyOn(
      await import("granted/plugin-sdk/session-store-runtime"),
      "resolveStorePath",
