...
function enableSlackAccount(cfg: GrantedConfig, accountId: string): GrantedConfig {
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: GrantedConfig; accountId: string }) =>
        inspectSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: GrantedConfig; accountId: string }) =>
        Object.entries(inspectSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: GrantedConfig; accountId: string }) =>
