...
function createSlackAccount(config: NonNullable<GrantedConfig["channels"]>["slack"]) {
  return {
    accountId: "default",
    enabled: true,
    botToken: "xoxb-test",
...
function createSlashCommandSlackConfig(): GrantedConfig {
  return {
    channels: {
      slack: {
        enabled: true,
...
async function collectSlackFindingsForConfig(cfg: GrantedConfig) {
