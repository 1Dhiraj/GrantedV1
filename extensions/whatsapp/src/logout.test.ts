...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    ...
    previousOAuthDir = process.env.GRANTED_OAUTH_DIR;
    process.env.GRANTED_OAUTH_DIR = path.join(fixtureRoot, "oauth");
    ({ logoutWeb } = await import("./auth-store.js"));
  });

  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      delete process.env.GRANTED_OAUTH_DIR;
    } else {
      process.env.GRANTED_OAUTH_DIR = previousOAuthDir;
    }
