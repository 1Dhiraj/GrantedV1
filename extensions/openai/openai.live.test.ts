...
const LIVE_MODEL_ID = process.env.GRANTED_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.6-luna";
const LIVE_IMAGE_MODEL = process.env.GRANTED_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.GRANTED_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-5.4-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.GRANTED_LIVE_TEST === "1";
