...
import { resolvePositiveTimeoutSeconds } from "granted/plugin-sdk/provider-web-fetch";
...
function resolveFirecrawlSearchConfig(cfg?: GrantedConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: GrantedConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: GrantedConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: GrantedConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
...
export function resolveFirecrawlOnlyMainContent(cfg?: GrantedConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: GrantedConfig, override?: number): number {
