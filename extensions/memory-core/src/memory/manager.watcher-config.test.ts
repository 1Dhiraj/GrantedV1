...
vi.mock("openclaw/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("granted/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
import { clearEmbeddingProviders as clearRegistry } from "granted/plugin-sdk/plugin-test-runtime";
...
describe("memory watcher config", () => {
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): GrantedConfig {
    const defaults: NonNullable<NonNullable<GrantedConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: GrantedConfig) {
