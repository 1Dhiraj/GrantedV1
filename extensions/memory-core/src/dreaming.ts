...
import { resolveMemoryDreamingPluginConfig } from "granted/plugin-sdk/memory-core-host-runtime-core";
import {
...
import { peekSystemEventEntries } from "granted/plugin-sdk/system-event-runtime";
...
type Logger = Pick<GrantedPluginApi["logger"], "info" | "warn" | "error">;
...
async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: GrantedPluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: GrantedPluginApi): void {
  ...
  const resolveCurrentConfig = (): GrantedConfig =>
