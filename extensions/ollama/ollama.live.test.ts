...
const LIVE = process.env.GRANTED_LIVE_TEST === "1" && process.env.GRANTED_LIVE_OLLAMA === "1";
const OLLAMA_BASE_URL =
  process.env.GRANTED_LIVE_OLLAMA_BASE_URL?.trim() || "http://127.0.0.1:11434";
const CHAT_MODEL = process.env.GRANTED_LIVE_OLLAMA_MODEL?.trim() || "llama3.2:latest";
const EMBEDDING_MODEL =
  process.env.GRANTED_LIVE_OLLAMA_EMBED_MODEL?.trim() || "embeddinggemma:latest";
const PROVIDER_ID = process.env.GRANTED_LIVE_OLLAMA_PROVIDER_ID?.trim() || "ollama-live-custom";
const RUN_WEB_SEARCH = process.env.GRANTED_LIVE_OLLAMA_WEB_SEARCH !== "0";
const RUN_EMBEDDINGS =
  process.env.GRANTED_LIVE_OLLAMA_EMBEDDINGS === "1" ||
  (process.env.GRANTED_LIVE_OLLAMA_EMBEDDINGS !== "0" && !isOllamaCloudBaseUrl(OLLAMA_BASE_URL));
...
function requireOllamaRuntimeApiKey(): string | undefined {
  ...
  if (!apiKey) {
    throw new Error(
      "GRANTED_LIVE_OLLAMA_BASE_URL points at a remote Ollama host; set OLLAMA_API_KEY.",
    );
  }
  return apiKey;
}
...
function buildCliEnv(root: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    GRANTED_LIVE_TEST: "1",
    GRANTED_LIVE_OLLAMA: "1",
    GRANTED_LIVE_OLLAMA_WEB_SEARCH: "0",
