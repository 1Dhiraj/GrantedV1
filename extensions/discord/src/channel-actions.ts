...
import { createUnionActionGate } from "granted/plugin-sdk/channel-actions";
import type {
...
function listDiscoverableDiscordAccounts(cfg: GrantedConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: GrantedConfig) {
