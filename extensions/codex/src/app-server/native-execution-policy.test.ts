...
import type { getSessionEntry as getSessionEntryType } from "granted/plugin-sdk/session-store-runtime";
...
vi.mock("openclaw/plugin-sdk/session-store-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("granted/plugin-sdk/session-store-runtime")>();
  return {
    ...actual,
