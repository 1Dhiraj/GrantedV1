 ...
 */
import type { GrantedPluginSecurityAuditContext } from "granted/plugin-sdk/plugin-entry";
...
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "granted/plugin-sdk/ssrf-policy";
import {
  hasNonEmptyString,
...
export function collectBrowserSecurityAuditFindings(ctx: GrantedPluginSecurityAuditContext) {
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.GRANTED_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.GRANTED_GATEWAY_PASSWORD) ||
