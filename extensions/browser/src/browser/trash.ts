...
import { movePathToTrash as movePathToTrashWithAllowedRoots } from "granted/plugin-sdk/browser-config";
...
export async function movePathToTrash(targetPath: string): Promise<string> {
  return await movePathToTrashWithAllowedRoots(targetPath, {
    // Managed browser data follows GRANTED_STATE_DIR/GRANTED_CONFIG_PATH, which
