...
const mocks = vi.hoisted(() => ({
  ...
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: GrantedConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<GrantedConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: GrantedConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): GrantedConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: GrantedConfig) {
