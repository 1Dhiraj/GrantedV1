...
import { isPidAlive, prepareOomScoreAdjustedSpawn } from "granted/plugin-sdk/process-runtime";
...
export async function launchOpenClawChrome(
  ...
  const spawnOnce = async (onStderr?: (chunk: Buffer | string) => void) => {
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredGrantedTmpDir(), ".chromium");
