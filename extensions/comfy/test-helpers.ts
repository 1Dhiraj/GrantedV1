...
export function buildComfyConfig(config: Record<string, unknown>): GrantedConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): GrantedConfig {
  return {
    models: {
      providers: {
        comfy: config,
