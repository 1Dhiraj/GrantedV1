...
describe("LM Studio embedding request headers", () => {
  it.each([
    ...
    vi.stubEnv("no_proxy", "127.0.0.1");
    vi.stubEnv("GRANTED_TEST_LMSTUDIO_LITERAL", "ambient-bait");
    vi.stubEnv("GRANTED_TEST_LMSTUDIO_PROVIDER", "resolved-provider-value");
    ...
    try {
      ...
      const { provider } = await createLmstudioEmbeddingProvider({
          ...
          models: {
            providers: {
              lmstudio: {
                ...
                headers: {
                  "X-Provider-Only": "${GRANTED_TEST_LMSTUDIO_PROVIDER}",
        ...
        remote: {
          ...
          headers: {
            "X-Already-Resolved": "  ${GRANTED_TEST_LMSTUDIO_LITERAL}  ",
