...
describe("mantis Slack desktop smoke runtime", () => {
  ...
  it("leases a desktop box, runs Slack QA inside it, copies artifacts, and stops on pass", async () => {
    ...
    const runtimeEnv = {
      ...
      GRANTED_QA_SLACK_CHANNEL_ID: "C123",
      GRANTED_QA_SLACK_DRIVER_BOT_TOKEN: "driver-token",
      GRANTED_QA_SLACK_SUT_APP_TOKEN: "app-token",
      GRANTED_QA_SLACK_SUT_BOT_TOKEN: "sut-token",
    };
    const runner = vi.fn(
    ...
    expect(
      commands.every((entry) => entry.env?.GRANTED_LIVE_OPENAI_KEY === "openai-runtime-key"),
    ).toBe(true);
    ...
    expect(remoteScript).toContain("GRANTED_MANTIS_SLACK_BROWSER_PROFILE_DIR");
    expect(remoteScript)
  ...
  it("runs approval checkpoint mode with default approval scenarios and records screenshots", async () => {
    ...
    expect(remoteScript).toContain('export GRANTED_QA_SLACK_CHANNEL_ID="$slack_channel_id"');
    ...
    expect(remoteScript).toContain("GRANTED_QA_SLACK_APPROVAL_CHECKPOINT_DIR");
    expect(remoteScript).toContain("GRANTED_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS");
    ...
    expect(remoteScript).toContain("GRANTED_MANTIS_APPROVAL_BROWSER_BIN");
  ...
  it.each([
    ...
    async ({ expectedScenarioIds, remoteTimeoutSeconds, requestedScenarioIds }) => {
      ...
      expect(remoteScript).toContain("GRANTED_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS:-470000");
      expect(remoteScript).toContain(
        `GRANTED_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS:-${remoteTimeoutSeconds}`,
      );
    },
  );
  ...
  it.each([
    ...
    async ({ expectedError, failure }) => {
      ...
      await expect(
        runMantisSlackDesktopSmoke({
          ...
          env: {
            CI: "1",
            OPENAI_API_KEY: "openai-runtime-key",
            GRANTED_QA_CONVEX_SECRET_CI: "convex-secret",
            GRANTED_QA_CONVEX_SITE_URL: "https://example.convex.site",
            PATH: process.env.PATH,
          },
          gatewaySetup: true,
          keepLease: false,
      ...
      expect(runCommand?.env?.GRANTED_MANTIS_SLACK_APP_TOKEN).toBe("xapp-leased");
      expect(runCommand?.env?.GRANTED_MANTIS_SLACK_BOT_TOKEN).toBe("xoxb-leased");
      expect(runCommand?.env?.GRANTED_MANTIS_SLACK_CHANNEL_ID).toBe("CLEASED");
      expect(runCommand?.env?.GRANTED_QA_SLACK_CHANNEL_ID).toBe("CLEASED");
      expect(runCommand?.env?.GRANTED_QA_SLACK_SUT_APP_TOKEN).toBe("xapp-leased");
      expect(runCommand?.env?.GRANTED_QA_SLACK_SUT_BOT_TOKEN).toBe("xoxb-leased");
  ...
  it("passes gateway setup when Crabbox returns non-zero after remote metadata proves success", async () => {
    ...
    const result = await runMantisSlackDesktopSmoke({
      ...
      env: {
        OPENAI_API_KEY: "openai-runtime-key",
        GRANTED_MANTIS_SLACK_APP_TOKEN: "xapp-direct",
        GRANTED_MANTIS_SLACK_BOT_TOKEN: "xoxb-direct",
        PATH: process.env.PATH,
      },
      gatewaySetup: true,
