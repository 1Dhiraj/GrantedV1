...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${yamlDoubleQuoted(context)}\n      dockerfile: Dockerfile\n      args:\n        GRANTED_EXTENSIONS: "${QA_DOCKER_PLUGIN_SELECTION}"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      GRANTED_CONFIG_PATH: /tmp/openclaw/openclaw.json
      GRANTED_STATE_DIR: /tmp/openclaw/state
...
1. Build the prebaked image once:
   - \`docker build -t openclaw:qa-local-prebaked --build-arg GRANTED_EXTENSIONS="${QA_DOCKER_PLUGIN_SELECTION}" -f Dockerfile .\`
2. Start the stack:
