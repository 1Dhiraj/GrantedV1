...
import { resolveModelRefFromString } from "granted/plugin-sdk/agent-runtime";
import { formatErrorMessage as formatQaErrorMessage } from "granted/plugin-sdk/error-runtime";
import { formatMemoryDreamingDay } from "granted/plugin-sdk/memory-core-host-status";
import { resolveSessionTranscriptsDirForAgent } from "granted/plugin-sdk/memory-host-core";
...
export async function runQaSuiteScenarioSteps(
  ...
  for (const step of steps) {
    try {
      if (process.env.GRANTED_QA_DEBUG === "1") {
