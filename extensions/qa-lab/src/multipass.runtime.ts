...
import { coerceErrorMessage, toStringifiedError } from "granted/plugin-sdk/error-runtime";
...
function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          ...
          key !== "GRANTED_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `GRANTED_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `GRANTED_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
