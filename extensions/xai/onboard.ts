...
function createXaiPresetAppliers(primaryModelRef: string) {
  return createModelCatalogPresetAppliers<["openai-completions" | "openai-responses"]>({
    primaryModelRef,
    resolveParams: (_cfg: GrantedConfig, api) => ({
      providerId: "xai",
      api,
...
function pruneRetiredXaiBuiltinModels(cfg: GrantedConfig): GrantedConfig {
...
export function applyXaiProviderConfig(cfg: GrantedConfig): GrantedConfig {
...
export function applyXaiConfig(cfg: GrantedConfig): GrantedConfig {
  return xaiPresetAppliers.applyConfig(pruneRetiredXaiBuiltinModels(cfg), "openai-responses");
}

export function applyXaiOAuthConfig(cfg: GrantedConfig): GrantedConfig {
