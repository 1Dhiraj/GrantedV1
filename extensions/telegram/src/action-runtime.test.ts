...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): GrantedConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as GrantedConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): GrantedConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  function topicCacheScopeFor(cfg: GrantedConfig, accountId: string): string {
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as GrantedConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: GrantedConfig, accountId = "media") {
    await handleTelegramAction(
