...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.GRANTED_SPARSE_PROFILE || process.env.GRANTED_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because GRANTED_A2UI_SKIP_MISSING=1 or GRANTED_SPARSE_PROFILE is set.",
      );
      return;
    }
