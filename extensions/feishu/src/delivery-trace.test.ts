...
vi.mock("./runtime.js", async () => {
  const replyChunking = await import("granted/plugin-sdk/reply-chunking");
  const textChunking = await import("granted/plugin-sdk/text-chunking");
  const markdownTables = await import("granted/plugin-sdk/markdown-table-runtime");
  const runtime = {
    media: {
      loadWebMedia: async () => {
