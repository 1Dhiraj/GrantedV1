...
import { resolveChannelConfigWrites } from "granted/plugin-sdk/channel-config-writes";
import { normalizeAccountId, resolveAgentRoute } from "granted/plugin-sdk/routing";
...
class DynamicAgentMutationSkipped extends Error {
  constructor(readonly cfg: GrantedConfig) {
...
export async function maybeCreateDynamicAgent(params: {
  ...
  accountId: string;
  senderOpenId: string;
  canCreateForConfig: (cfg: GrantedConfig) => Promise<boolean>;
  log: (msg: string) => void;
  ...
  const currentCfg = runtime.config.current() as GrantedConfig;
  ...
  let skippedCfg: GrantedConfig | undefined;
  ...
  return {
    ...
    updatedCfg: runtime.config.current() as GrantedConfig,
