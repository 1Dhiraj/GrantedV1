...
export function createToolFactoryHarness(cfg: GrantedPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<GrantedPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
