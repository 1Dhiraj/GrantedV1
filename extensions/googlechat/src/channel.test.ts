...
function resolveGoogleChatAccountImpl(params: { cfg: GrantedConfig; accountId?: string | null }) {
...
vi.mock("./channel.deps.runtime.js", () => {
  return {
    ...
    listGoogleChatAccountIds: (cfg: GrantedConfig) => {
...
function createGoogleChatCfg(): GrantedConfig {
  return {
    channels: {
      googlechat: {
        enabled: true,
