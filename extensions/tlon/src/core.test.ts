...
const tlonTestPlugin = {
  ...
  setup: {
    resolveAccountId: ({ accountId }: { cfg: GrantedConfig; accountId?: string | null }) =>
      accountId ?? "default",
  },
};
...
describe("tlon core", () => {
  ...
  it("does not invent an account when the Tlon channel is unconfigured", () => {
    expect(listTlonAccountIds({} as GrantedConfig)).toEqual([]);
    expect(listTlonAccountIds({ channels: { tlon: {} } } as GrantedConfig)).toEqual([]);
  });
