...
export function enforceSupportedNodeRuntime(
  ...
  const requirement = engine
    ? `this Granted release requires Node ${engine}.`
    : "could not read this Granted release's Node requirement.";
  reportError(
    [
      `[granted] error: ${requirement}`,
      `[granted] detected Node ${detectedRuntime?.version ?? "missing"} (exec: ${detectedRuntime?.execPath || "unknown"}).`,
      "[granted] install Node: https://nodejs.org/en/download",
      "[granted] upgrade Node, then retry the Granted update.",
    ].join("\n"),
  );
  return false;
}
...
export function removeLegacyPackageInstallGuard(
  ...
  try {
    ...
    reportError(
      `[granted] error: could not remove the legacy package install guard: ${
...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[granted] warning: detected ${normalizedPackageManager} for install lifecycle.`,
    "[granted] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[granted] prefer: corepack pnpm install",
  ].join("\n");
}
