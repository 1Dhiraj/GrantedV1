...
// Usage:
// GRANTED_PROMPT_TRANSPORT=direct|gateway
// GRANTED_PROMPT_MODE=extra
// GRANTED_PROMPT_TEXT='...'
// GRANTED_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.GRANTED_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE = "extra";
const PROMPT_TEXT = process.env.GRANTED_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.GRANTED_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.GRANTED_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "GRANTED_PROMPT_CAPTURE",
  raw: process.env.GRANTED_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "GRANTED_PROMPT_INCLUDE_RAW",
  raw: process.env.GRANTED_PROMPT_INCLUDE_RAW,
});
const KEEP_TMP = parseBooleanEnv({
  fallback: false,
  name: "GRANTED_PROMPT_KEEP_TMP",
  raw: process.env.GRANTED_PROMPT_KEEP_TMP,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.GRANTED_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "GRANTED_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.GRANTED_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "GRANTED_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.GRANTED_PROMPT_GATEWAY_TIMEOUT_MS,
});
const CAPTURE_PROXY_MAX_BODY_BYTES = parseStrictIntegerOption({
  fallback: 2 * 1024 * 1024,
  label: "GRANTED_PROMPT_CAPTURE_MAX_BODY_BYTES",
  min: 1,
  raw: process.env.GRANTED_PROMPT_CAPTURE_MAX_BODY_BYTES,
});
const GATEWAY_LOG_TAIL_BYTES = 256 * 1024;
const SETUP_TOKEN_RAW = process.env.GRANTED_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.GRANTED_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.GRANTED_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
...
async function resolveSetupTokenSource(): Promise<TokenSource> {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set GRANTED_LIVE_SETUP_TOKEN_VALUE or GRANTED_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
  ...
  const child = spawn(
    ...
    ["openclaw.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      ...
      env: {
        ...
        GRANTED_GATEWAY_TOKEN: params.gatewayToken,
        ...
        GRANTED_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      detached: process.platform !== "win32",
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing GRANTED_PROMPT_TEXT or GRANTED_PROMPT_LIST_JSON");
  }
