...
const ENV_VAR_PATTERN = /GRANTED_[A-Z0-9_]+/gu;
...
export function main(argv: string[] = process.argv.slice(2), root = process.cwd()) {
  ...
  if (baseBudget !== null) {
    enforceRatchetScalar(budget, baseBudget, {
      increased: `GRANTED_* budget grew from ${baseBudget} to ${budget}`,
    });
  }
  const names = collectEnvVarNames(root, { staged });
  enforceRatchetScalar(names.length, budget, {
    decreased: `GRANTED_* count ${names.length} is below budget ${budget}; update ${BUDGET_PATH}`,
    increased: `GRANTED_* count ${names.length} exceeds budget ${budget}; update ${BUDGET_PATH}`,
  });
  reportRatchetSuccess(`GRANTED_* count ${names.length}/${budget}`);
  return names.length;
}
