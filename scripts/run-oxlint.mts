...
const GRANTED_FOCUSED_CONFIG_FLAG = "--openclaw-focused-config";
...
async function runOxlint(
  ...
  const focusedConfig = argv.includes(GRANTED_FOCUSED_CONFIG_FLAG);
  const oxlintArgs = argv.filter((arg) => arg !== GRANTED_FOCUSED_CONFIG_FLAG);
  ...
  const needsArtifactPreparation =
    !focusedConfig &&
    env.GRANTED_OXLINT_SKIP_PREPARE !== "1" &&
...
if (isDirectRunUrl(process.argv[1], import.meta.url)) {
  ...
  process.exitCode =
    !argv.includes(GRANTED_FOCUSED_CONFIG_FLAG) &&
