...
export function resolveUiBuildEnvironment(
  ...
  if (env.GRANTED_BUILD_TIMESTAMP?.trim() || env.GRANTED_CONTROL_UI_BUILD_ID?.trim()) {
    return buildEnv;
  }
  ...
  const release = env.GRANTED_CONTROL_UI_RELEASE_BUILD?.trim() === "1";
  if (
  ...
  return {
    ...buildEnv,
    GRANTED_BUILD_TIMESTAMP: existing.builtAt,
    GRANTED_CONTROL_UI_BUILD_ID: existing.buildId,
  };
}
...
export function runUiCli(argv: string[] = process.argv.slice(2)): void {
  ...
  const noPnpmBuild = action === "build" && process.env.GRANTED_BUILD_ALL_NO_PNPM === "1";
