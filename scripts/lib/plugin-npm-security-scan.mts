...
const CURRENT_REQUIRED_REVIEWED_SOURCE_FINDING_COUNTS = new Map<string, number>([
  ["@granted/acpx:dangerous-exec:src/codex-auth-bridge.ts", 1],
  ["@granted/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs", 1],
  ["@granted/codex:dangerous-exec:src/app-server/transport-stdio.ts", 1],
  ["@granted/codex:dangerous-exec:src/doctor.ts", 1],
  ["@granted/discord:dangerous-exec:src/voice/audio.ts", 1],
  ["@granted/imessage:dangerous-exec:src/client.ts", 1],
  ["@granted/llama-cpp-provider:dangerous-exec:src/llama-server-install.ts", 1],
  ["@granted/mxc-sandbox:dangerous-exec:src/readiness.ts", 2],
  ["@granted/raft:dangerous-exec:src/gateway.ts", 1],
  ["@granted/signal:dangerous-exec:src/daemon.ts", 1],
  ["@granted/voice-call:dangerous-exec:src/tunnel.ts", 1],
]);
...
const CURRENT_REVIEWED_RELEASE_LAYOUT = {
  ...
  findings: new Map<string, number>([
    ["@granted/codex:dangerous-exec:src/app-server/sandbox-exec-server/sandbox-child.ts", 1],
    ["@granted/codex:dangerous-exec:src/app-server/transport-process-snapshot.ts", 1],
  ]),
};

const CURRENT_OPTIONAL_REVIEWED_PACKED_FINDING_COUNTS = new Map<string, number>([
  ["@granted/acpx:dangerous-exec:dist/mcp-proxy.mjs", 1],
  ["@granted/acpx:dangerous-exec:dist/service-<hash>.js", 1],
  ["@granted/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.test.ts", 3],
  ["@granted/codex:dangerous-exec:dist/api.js", 1],
  ["@granted/codex:dangerous-exec:dist/dynamic-tools-<hash>.js", 2],
  ["@granted/codex:dangerous-exec:dist/session-catalog-<hash>.js", 1],
  ["@granted/codex:dangerous-exec:dist/transport-stdio-<hash>.js", 1],
  ["@granted/codex:dangerous-exec:src/app-server/attempt-startup-retry.test.ts", 6],
  ["@granted/codex:dangerous-exec:src/app-server/sandbox-exec-server.http.test.ts", 1],
  ["@granted/codex:dangerous-exec:src/app-server/transport-orphan.test-helper.ts", 1],
  ["@granted/codex:dangerous-exec:src/app-server/transport-orphan.test.ts", 3],
  ["@granted/codex:dangerous-exec:src/app-server/transport-process-snapshot.test.ts", 1],
  ["@granted/codex:dangerous-exec:src/app-server/transport-startup.test.ts", 2],
  ["@granted/codex:dangerous-exec:src/app-server/transport.process.test.ts", 10],
  ["@granted/diagnostics-prometheus:dangerous-exec:src/install-runtime.e2e.test.ts", 2],
  ["@granted/google-meet:dangerous-exec:src/cli-artifacts.test.ts", 1],
  ["@granted/google-meet:dangerous-exec:src/realtime.process.test.ts", 1],
  ["@granted/imessage:dangerous-exec:src/client.test.ts", 3],
  ["@granted/llama-cpp-provider:dangerous-exec:dist/index.js", 1],
  ["@granted/memory-lancedb:dangerous-exec:memory-lancedb.concurrent.test.ts", 1],
  ["@granted/opencode-go-provider:env-harvesting:opencode-go.live.test.ts", 1],
  ["@granted/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js", 1],
  ["@granted/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js", 1],
]);
...
const FROZEN_RELEASE_REQUIRED_REVIEWED_SOURCE_FINDING_COUNTS = new Map<string, number>([
  ...
  ["@granted/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs", 1],
  ["@granted/codex:dangerous-exec:src/app-server/transport-stdio.ts", 1],
  ["@granted/codex:dangerous-exec:src/node-cli-sessions.ts", 1],
  ["@granted/discord:dangerous-exec:src/voice/audio.ts", 1],
  ["@granted/google-meet:dangerous-exec:src/node-host.ts", 3],
  ["@granted/google-meet:dangerous-exec:src/realtime.ts", 2],
  ["@granted/matrix:dangerous-exec:src/matrix/deps.ts", 1],
  ["@granted/raft:dangerous-exec:src/gateway.ts", 1],
  ["@granted/signal:dangerous-exec:src/daemon.ts", 1],
  ["@granted/voice-call:dangerous-exec:src/tunnel.ts", 4],
  ["@granted/voice-call:dangerous-exec:src/webhook/tailscale.ts", 1],
]);

const FROZEN_RELEASE_OPTIONAL_REVIEWED_PACKED_FINDING_COUNTS = new Map<string, number>([
  ["@granted/acpx:dangerous-exec:dist/mcp-proxy.mjs", 1],
  ["@granted/acpx:dangerous-exec:dist/service-<hash>.js", 1],
  ["@granted/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.test.ts", 1],
  ["@granted/codex:dangerous-exec:dist/client-<hash>.js", 1],
  ["@granted/codex:dangerous-exec:src/app-server/sandbox-exec-server.http.test.ts", 1],
  ["@granted/google-meet:dangerous-exec:dist/index.js", 1],
  ["@granted/google-meet:dangerous-exec:src/realtime.process.test.ts", 1],
  ["@granted/openshell-sandbox:dangerous-exec:src/backend.e2e.test.ts", 1],
  ["@granted/openshell-sandbox:dangerous-exec:src/openshell-core.test.ts", 2],
  ["@granted/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js", 1],
  ["@granted/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js", 1],
]);
...
const FROZEN_EXTENDED_STABLE_2026_6_33_LAYOUT = {
  ...
  findings: new Map<string, number>([
    ["@granted/codex:dangerous-exec:src/app-server/sandbox-exec-server/http.ts", 1],
    ["@granted/codex:dangerous-exec:src/app-server/sandbox-exec-server/processes.ts", 1],
  ]),
};
