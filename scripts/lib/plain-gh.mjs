...
export function plainGhAuthenticatedEnv(env) {
  ...
  const tokenEnv = { ...next };
  delete tokenEnv.GRANTED_GH_BIN;
...
export function resolvePlainGhBin(env = process.env) {
  if (env.GRANTED_GH_BIN) {
    if (isExecutable(env.GRANTED_GH_BIN)) {
      return env.GRANTED_GH_BIN;
    }
    throw new Error(`GRANTED_GH_BIN is not executable: ${env.GRANTED_GH_BIN}`);
  }
