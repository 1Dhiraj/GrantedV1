...
export function validatePackageSource({
  ...
  const aiDependency = rootDependencies["@granted/ai"];
  if (aiManifestContent === null) {
    if (aiDependency === undefined) {
      return rootManifest.version;
    }
    throw new Error(`${ROOT_MANIFEST_PATH} declares @granted/ai without ${AI_MANIFEST_PATH}.`);
  }

  const aiManifest = parseManifest(aiManifestContent, AI_MANIFEST_PATH);
  if (aiDependency !== "workspace:*") {
    throw new Error(
      `${ROOT_MANIFEST_PATH} must depend on @granted/ai via workspace:*; found ${JSON.stringify(aiDependency)}.`,
    );
  }
