...
const CRABBOX_GATE_TEST_ENV =
  "CI=1 NODE_OPTIONS=--max-old-space-size=4096 GRANTED_VITEST_MAX_WORKERS=1";
...
export function buildCrabboxGateCommand(plan, bootstrapSha256) {
  ...
  return [
    "set -euo pipefail",
    "umask 022",
    `printf '%s\\n' 'GRANTED_CRABBOX_GATE_VERSION=1' 'GRANTED_CRABBOX_GATE_MODE=remote_crabbox_aws' 'GRANTED_CRABBOX_GATE_BASE=${validated.baseSha}' 'GRANTED_CRABBOX_GATE_HEAD=${validated.headSha}' 'GRANTED_CRABBOX_GATE_PLAN_SHA256=${planDigest}' 'GRANTED_CRABBOX_GATE_TARGET_COUNT=${validated.targets.length}' 'GRANTED_CRABBOX_BOOTSTRAP_SHA256=${bootstrapSha256}'`,
    "printf '%s\\n' 'GRANTED_CRABBOX_GATE_STAGE=build:start'",
    "pnpm build",
    "printf '%s\\n' 'GRANTED_CRABBOX_GATE_STAGE=build:ok' 'GRANTED_CRABBOX_GATE_STAGE=check:start'",
    "pnpm check",
    "printf '%s\\n' 'GRANTED_CRABBOX_GATE_STAGE=check:ok' 'GRANTED_CRABBOX_GATE_STAGE=test:start'",
    testCommand,
    "printf '%s\\n' 'GRANTED_CRABBOX_GATE_STAGE=test:ok' 'GRANTED_CRABBOX_GATE_RESULT=success'",
  ].join("; ");
}
