...
const DEFAULT_COMMAND_TIMEOUT_MS = readPositiveInt(
  process.env.GRANTED_RELEASE_BETA_SMOKE_COMMAND_MS,
  10 * 60_000,
  "GRANTED_RELEASE_BETA_SMOKE_COMMAND_MS",
);
const TELEGRAM_POLL_INTERVAL_MS = readPositiveInt(
  process.env.GRANTED_RELEASE_BETA_SMOKE_POLL_INTERVAL_MS,
  30_000,
  "GRANTED_RELEASE_BETA_SMOKE_POLL_INTERVAL_MS",
);
const TELEGRAM_POLL_TIMEOUT_MS = readPositiveInt(
  process.env.GRANTED_RELEASE_BETA_SMOKE_POLL_TIMEOUT_MS,
  4 * 60 * 60_000,
  "GRANTED_RELEASE_BETA_SMOKE_POLL_TIMEOUT_MS",
);
  ...
  const result = spawnSync(
    ...
    [
      ...
      [
        "set -euo pipefail",
        'token="$(gh auth token)"',
        'curl -fsS -H "Authorization: Bearer ${token}" -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" "${GRANTED_GITHUB_REST_URL}"',
      ].join("\n"),
    ],
    {
      encoding: "utf8",
      env: { ...process.env, GRANTED_GITHUB_REST_URL: url },
      killSignal: "SIGKILL",
      maxBuffer: CAPTURE_MAX_BUFFER_BYTES,
