...
if (mode === "assert-release-versions") {
  ...
  const rootAiVersion = rootManifest.dependencies?.["@granted/ai"];
  if (
    ...
    throw new Error(
      `candidate version mismatch: openclaw=${String(rootVersion)}, dependency=${String(rootAiVersion)}, @granted/ai=${String(aiVersion)}`,
    );
  }
...
if (mode === "assert-openclaw-trusted") {
  ...
  const legacyPendingPath = path.join(packageRoot, `.${legacyProjectName}-lifecycle-pending`);
  const legacyGuardPath = path.join(packageRoot, "dist", `${legacyProjectName}-install-guard`);
  if (
