...
function writeConfig(kind) {
  const configPath = requireArg(process.env.GRANTED_CONFIG_PATH, "GRANTED_CONFIG_PATH");
      ...
      : kind === "browser-cdp"
        ? {
            gateway: {
              ...
              auth: {
                mode: "token",
                token: requireArg(process.env.GRANTED_GATEWAY_TOKEN, "GRANTED_GATEWAY_TOKEN"),
              },
              controlUi: { enabled: false },
            },
            browser: {
...
function writeOpenAiWebSearchMinimalConfig() {
  writeJson(path.join(process.env.GRANTED_STATE_DIR, "openclaw.json"), {
    agents: {
      defaults: {
    ...
    gateway: { auth: { mode: "token", token: process.env.GRANTED_GATEWAY_TOKEN } },
  });
}

function writeOpenWebUiConfig([openaiApiKey]) {
  const batchPath = requireArg(process.env.GRANTED_CONFIG_BATCH_PATH, "GRANTED_CONFIG_BATCH_PATH");
  writeJson(batchPath, [
    ...
    { path: "models.providers.openai.models", value: [] },
    {
      ...
      value: readPositiveIntEnv("GRANTED_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS", 900),
    },
    ...
    { path: "gateway.auth.token", value: process.env.GRANTED_GATEWAY_TOKEN },
    { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
    { path: "agents.defaults.model.primary", value: process.env.GRANTED_OPENWEBUI_MODEL },
  ]);
}
