...
export function normalizeBuildTimestamp(raw: string, source = "GRANTED_BUILD_TIMESTAMP"): string {
...
export function resolveBuildInfo(options: ResolveBuildInfoOptions = {}): BuildInfo {
  ...
  const explicitTimestamp = env.GRANTED_BUILD_TIMESTAMP?.trim();
  ...
  const releaseFlag = env.GRANTED_CONTROL_UI_RELEASE_BUILD?.trim();
  if (releaseFlag && releaseFlag !== "1") {
    throw new Error("GRANTED_CONTROL_UI_RELEASE_BUILD must be 1 when set");
  }
