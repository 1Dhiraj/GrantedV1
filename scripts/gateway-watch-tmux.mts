...
const TMUX_CWD_ENV_KEY = "GRANTED_GATEWAY_WATCH_CWD";
...
const TMUX_CHILD_ENV_KEYS = [
  ...
  "GRANTED_DIAGNOSTICS",
  "GRANTED_DIAGNOSTICS_EVENT_LOOP",
  "GRANTED_DIAGNOSTICS_TIMELINE_PATH",
  "GRANTED_GATEWAY_PORT",
  "GRANTED_GATEWAY_RESTART_TRACE",
  "GRANTED_GATEWAY_STARTUP_TRACE",
  "GRANTED_GATEWAY_WATCH_AUTO_DOCTOR",
  "GRANTED_HOME",
  "GRANTED_PROFILE",
...
const resolveGatewayWatchPort = (args: string[], env: NodeJS.ProcessEnv) => {
  ...
  return {
    explicitCli: false,
    port: parsePortValue(env.GRANTED_GATEWAY_PORT, { allowHost: true }),
  };
};
...
const resolveGatewayWatchProfile = (args: string[], env: NodeJS.ProcessEnv) => {
  ...
  return env.GRANTED_PROFILE || null;
};
...
const resolveGatewayWatchBenchmarkArgs = ({
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.GRANTED_TRACE_SYNC_IO ??= "0";
    if (nextEnv.GRANTED_TRACE_SYNC_IO === "1") {
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    ...
    `GRANTED_GATEWAY_WATCH_TMUX_CHILD=1`,
    `GRANTED_GATEWAY_WATCH_SESSION=${sessionName}`,
...
export const runGatewayWatchServiceHandoff = (params: GatewayWatchParams = {}): number => {
  ...
  const stopEnv = { ...env };
  if (explicitCli) {
    stopEnv.GRANTED_GATEWAY_PORT = String(watchPort);
  }
...
const shouldAttachTmux = (deps: Pick<GatewayWatchDeps, "env" | "stdinIsTTY" | "stdoutIsTTY">) => {
  const raw = (deps.env.GRANTED_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params: GatewayWatchParams = {}): number => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.GRANTED_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
    return runForegroundWatcher(deps);
  }

  if (deps.env.GRANTED_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher(deps);
  }
