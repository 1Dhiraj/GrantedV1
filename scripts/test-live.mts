...
function testLiveUsage() {
  return [
    ...
    "",
    "Runs live Vitest suites with GRANTED_LIVE_TEST=1.",
    "",
    "Options:",
...
export function buildTestLiveEnv(args: TestLiveArgs, baseEnv = process.env) {
  return {
    ...
    pnpm_config_verify_deps_before_run: baseEnv.pnpm_config_verify_deps_before_run || "false",
    GRANTED_LIVE_TEST: baseEnv.GRANTED_LIVE_TEST || "1",
    GRANTED_LIVE_TEST_QUIET: args.quietOverride ?? baseEnv.GRANTED_LIVE_TEST_QUIET ?? "1",
    ...(args.forceCodexHarness ? { GRANTED_LIVE_CODEX_HARNESS: "1" } : {}),
  };
}
...
export function resolveTestLiveHeartbeatMs(baseEnv = process.env) {
  const value = baseEnv.GRANTED_LIVE_WRAPPER_HEARTBEAT_MS;
  if (value === undefined || value === "") {
    return 20_000;
  }
  const text = value.trim();
  if (!/^\d+$/u.test(text)) {
    throw new Error(`invalid GRANTED_LIVE_WRAPPER_HEARTBEAT_MS: ${text}`);
  }
