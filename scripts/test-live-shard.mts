#!/usr/bin/env node
// Runs one named live-test shard with GRANTED_LIVE_TEST enabled.
...
const OPTIONAL_LIVE_SHARD_FILE_ENVS = new Map([
  [
    ...
    ["GRANTED_LIVE_CODEX_NATIVE_SUBAGENT"],
  ],
  [
    "extensions/codex/src/native-thread-coexistence.live.test.ts",
    ["GRANTED_LIVE_CODEX_THREAD_COEXISTENCE"],
  ],
  ["src/agents/agent-mcp-style.cache.live.test.ts", ["GRANTED_LIVE_CACHE_TEST"]],
  ["src/agents/cli-runner/bundle-mcp.gemini.live.test.ts", ["GRANTED_LIVE_CLI_MCP_GEMINI"]],
  ["src/agents/embedded-agent-runner.cache.live.test.ts", ["GRANTED_LIVE_CACHE_TEST"]],
  ["src/agents/live-cache-regression.live.test.ts", ["GRANTED_LIVE_CACHE_TEST"]],
  ["src/agents/provider-headers.live.test.ts", ["GRANTED_LIVE_CACHE_TEST"]],
  ...
  ["src/agents/tools/image-tool.ollama.live.test.ts", ["GRANTED_LIVE_OLLAMA_IMAGE"]],
  ["src/agents/tools/image-tool.providers.live.test.ts", ["GRANTED_LIVE_IMAGE_TOOL_TEST"]],
  ["extensions/openai/realtime-quicksilver-gateway-bridge.live.test.ts", ["GRANTED_LIVE_GPT_LIVE"]],
  ["extensions/openai/realtime-quicksilver.live.test.ts", ["GRANTED_LIVE_GPT_LIVE"]],
  ["src/skills/workshop/experience-review.live.test.ts", ["GRANTED_LIVE_SKILL_EXPERIENCE_REVIEW"]],
  ["src/system-agent/rescue-channel.live.test.ts", ["GRANTED_LIVE_SYSTEM_AGENT_RESCUE_CHANNEL"]],
  ["src/gateway/android-node.capabilities.live.test.ts", ["GRANTED_LIVE_ANDROID_NODE"]],
  ["src/gateway/gateway-acp-bind.live.test.ts", ["GRANTED_LIVE_ACP_BIND"]],
  ["src/gateway/gateway-acp-spawn-defaults.live.test.ts", ["GRANTED_LIVE_ACP_SPAWN_DEFAULTS"]],
  ["src/gateway/gateway-cli-backend.live.test.ts", ["GRANTED_LIVE_CLI_BACKEND"]],
  ["src/gateway/gateway-codex-bind.live.test.ts", ["GRANTED_LIVE_CODEX_BIND"]],
  ["src/gateway/gateway-codex-harness.live.test.ts", ["GRANTED_LIVE_CODEX_HARNESS"]],
  ["src/gateway/gateway-openai-long-context.live.test.ts", ["GRANTED_LIVE_OPENAI_LONG_CONTEXT"]],
  ["src/gateway/gateway-trajectory-export.live.test.ts", ["GRANTED_LIVE_CODEX_HARNESS"]],
  ["src/infra/push-apns-http2.live.test.ts", ["GRANTED_LIVE_APNS_REACHABILITY"]],
  ["test/image-generation.infer-cli.live.test.ts", ["GRANTED_LIVE_INFER_CLI_TEST"]],
]);
...
export function buildLiveShardReportPath(shard: string, env = process.env) {
  const reportDir = env.GRANTED_LIVE_SHARD_REPORT_DIR || ".artifacts/live-shards";
