...
describe("syncPluginVersions", () => {
  ...
  it("preserves workspace openclaw devDependencies and plugin host floors", () => {
    ...
    expect(summary.updated).toContain("@granted/imessage");
    expect(summary.updated).toContain("@granted/ai");
    expect(summary.updated).not.toContain("@granted/llm-core");
    expect(
  ...
  it("reports pending version sync without writing in check mode", () => {
    ...
    expect(summary.updated).toEqual(["@granted/discord"]);
  ...
  it("uses the base release version for beta changelog entries", () => {
    ...
    expect(summary.changelogged).toEqual(["@granted/matrix"]);
