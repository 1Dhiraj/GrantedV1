...
function inspection(running = false): Extract<FleetContainerInspectResult, { kind: "ok" }> {
  return {
    ...
    environment: { GRANTED_GATEWAY_TOKEN: "old-token" },
...
describe("fleet restore runtime", () => {
  ...
  it("swaps state, repins config, and rotates the token", async () => {
    ...
    expect(containers.run.mock.calls[0]?.[0].environment.GRANTED_GATEWAY_TOKEN).toBe("new-token");
    ...
    expect(containers.run.mock.calls[0]?.[0].environment.GRANTED_GATEWAY_TOKEN).not.toBe(
      "old-token",
    );
