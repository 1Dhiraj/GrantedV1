...
const resolveRoute = (
  params: Omit<Parameters<typeof resolveAgentRoute>[0], "cfg"> & { cfg?: GrantedConfig },
) =>
  resolveAgentRoute({
    cfg: params.cfg ?? {},
    ...params,
...
describe("resolveAgentRoute", () => {
  ...
  const expectRouteResolutionCase = (params: {
    routeParams: Omit<Parameters<typeof resolveRoute>[0], "cfg"> & { cfg: GrantedConfig };
    expected: ResolvedRouteExpectation;
  }) => {
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<GrantedConfig["bindings"]>[number];
...
describe("unknown direct-message route decisions", () => {
  ...
  test.each([
    ...
    const bindings: NonNullable<GrantedConfig["bindings"]> = [
      {
        agentId: "exact",
