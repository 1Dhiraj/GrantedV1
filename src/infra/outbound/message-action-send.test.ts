...
describe("runMessageAction plugin dispatch", () => {
  ...
  describe("alias-based plugin action dispatch", () => {
    ...
    it("stages workspace-reader media before gateway dispatch", async () => {
      ...
      await runMessageAction({
        cfg: { channels: { gatewaychat: { enabled: true } } } as GrantedConfig,
        action: "send",
        params: {
          channel: "gatewaychat",
          target: "user-123",
  ...
  describe("presentation send routing", () => {
    const handleAction = vi.fn(
      async ({ cfg, params }: { cfg: GrantedConfig; params: Record<string, unknown> }) => {
    ...
    it.each([
      ...
      await runMessageAction({
        cfg: { channels: { cardchat: { enabled: true } } } as GrantedConfig,
        action: "send",
