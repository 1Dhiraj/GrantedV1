...
export class DirectoryCache<T> {
  private cachesByConfig = new WeakMap<GrantedConfig, Map<string, CacheEntry<T>>>();
   ...
   */
  get(key: string, cfg: GrantedConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: GrantedConfig): void {
    const cache = this.cacheForConfig(cfg);
    const now = Date.now();
   ...
   */
  clearMatching(match: (key: string) => boolean, cfg: GrantedConfig): void {
  ...
  private cacheForConfig(cfg: GrantedConfig): Map<string, CacheEntry<T>> {
