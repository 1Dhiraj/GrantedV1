...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "granted.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTestDir({ prefix: "openclaw-watch-node-" }, async (cwd) => {
      ...
      expect(spawnEnv.GRANTED_WATCH_MODE).toBe("1");
      expect(spawnEnv.GRANTED_WATCH_SESSION).toBe("1700000000000-4242");
      expect(spawnEnv.GRANTED_NO_RESPAWN).toBe("1");
      expect(spawnEnv.GRANTED_WATCH_COMMAND).toBe("gateway --force");
      expect(spawnEnv.GRANTED_TRACE_SYNC_IO).toBeUndefined();
  ...
  it("preserves explicit sync I/O trace overrides for gateway watch", async () => {
    ...
    await withTestDir({ prefix: "openclaw-watch-node-" }, async (cwd) => {
      ...
      expect(spawnCall[0]).toBe("/usr/local/bin/node");
      expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
      expect(requireSpawnEnv(spawn, 0).GRANTED_TRACE_SYNC_IO).toBe("0");
  ...
  it("does not run doctor after a gateway failure when auto doctor is disabled", async () => {
    ...
    const runPromise = runWatch({
      args: ["gateway", "--force"],
      createWatcher,
      env: { GRANTED_GATEWAY_WATCH_AUTO_DOCTOR: "0" },
      lockDisabled: true,
      process: fakeProcess,
      spawn,
    });
