...
export function assertGatewayServiceMutationAllowed(
  ...
  if (serviceIdentityConflict) {
    ...
    throw new Error(
      `service management skipped: named profiles cannot override ${serviceIdentityConflict.envKey} for ${platformName} service management. Unset ${serviceIdentityConflict.envKey} so OpenClaw derives the native service identity from GRANTED_PROFILE to ${action}, or keep this profile runtime-only without a native service.`,
    );
  }
  if (!isDefaultInstallIdentity(env)) {
    throw new Error(
      `${NON_DEFAULT_INSTALL_SERVICE_SKIP_REASON}. Rerun with HOME set to the OS account home, without GRANTED_HOME, and with GRANTED_STATE_DIR and GRANTED_CONFIG_PATH either unset or pointing at the canonical paths for that account home and profile to ${action}.`,
    );
  }
}
