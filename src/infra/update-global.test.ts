...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["GRANTED_UPDATE_PACKAGE_SPEC"]);
    process.env.GRANTED_UPDATE_PACKAGE_SPEC = "file:/tmp/openclaw.tgz";
    ...
    expect(
      resolveGlobalInstallSpec({
        ...
        tag: "beta",
        env: { GRANTED_UPDATE_PACKAGE_SPEC: "openclaw@next" },
      }),
    ).toBe("openclaw@next");
  });
  ...
  it.each([
    ...
    { packageName: "openclaw", spec: "openclaw@=v1.2.3", expected: "1.2.3" },
    {
      packageName: "@granted/core",
      spec: "@granted/core@2026.7.30-beta.1",
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTestDir({ prefix: "openclaw-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "telegram", "@granted/telegram");
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTestDir(
      ...
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@granted/qa-lab");
        await writePackageDistInventory(packageRoot);
