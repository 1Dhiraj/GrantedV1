...
describe("startProxy", () => {
  ...
  it("uses GRANTED_PROXY_URL when config proxyUrl is omitted", async () => {
    process.env["GRANTED_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("honors an explicit opt-out when GRANTED_PROXY_URL is present", async () => {
    process.env["GRANTED_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("prefers config proxyUrl over GRANTED_PROXY_URL", async () => {
    process.env["GRANTED_PROXY_URL"] = "http://127.0.0.1:3128";
  ...
  it("uses HTTPS proxy URLs from GRANTED_PROXY_URL", async () => {
    process.env["GRANTED_PROXY_URL"] = "https://127.0.0.1:3128";
  ...
  it("passes configured proxy CA trust to Proxyline", async () => {
    ...
    expect(process.env["GRANTED_PROXY_CA_FILE"]).toBe(caFile);
    expect(installGlobalProxyMock).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("loads inherited HTTPS proxy CA trust for child routing", () => {
    ...
    process.env["GRANTED_PROXY_LOOPBACK_MODE"] = "gateway-only";
  ...
  it("sets process proxy env vars for inherited clients", async () => {
    ...
    expect(process.env["GRANTED_PROXY_LOOPBACK_MODE"]).toBe("gateway-only");
  });
  ...
  it("persists loopbackMode in env for forked child CLIs", async () => {
    ...
    expect(process.env["GRANTED_PROXY_LOOPBACK_MODE"]).toBe("block");
    expect(getActiveManagedProxyLoopbackMode()).toBe("block");

    await stopProxy(handle);
    process.env["GRANTED_PROXY_ACTIVE"] = "1";
    process.env["GRANTED_PROXY_LOOPBACK_MODE"] = "proxy";
  ...
  it("restores previous proxy env and stops Proxyline on stop", async () => {
    ...
    expect(process.env["GRANTED_PROXY_ACTIVE"]).toBeUndefined();
    expect(proxylineStopMock).toHaveBeenCalledOnce();
    expect(forceResetGlobalDispatcherMock).toHaveBeenCalledTimes(2);
  });
