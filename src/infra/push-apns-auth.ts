...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.GRANTED_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.GRANTED_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set GRANTED_APNS_TEAM_ID and GRANTED_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.GRANTED_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.GRANTED_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.GRANTED_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set GRANTED_APNS_PRIVATE_KEY_P8 or GRANTED_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading GRANTED_APNS_PRIVATE_KEY_PATH (${keyPath}): ${formatErrorMessage(err)}`,
    };
  }
}
