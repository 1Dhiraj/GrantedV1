...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers GRANTED_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands GRANTED_HOME when set to ~", () => {
    const env = {
      GRANTED_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
  ...
  it("does not interpret $ patterns in HOME when expanding GRANTED_HOME tilde", () => {
    const env = {
      GRANTED_HOME: "~/state",
      HOME: "/home/$&user",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for GRANTED_HOME",
      env: { GRANTED_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when GRANTED_HOME is tilde-only and no fallback home exists",
      env: { GRANTED_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
  ...
  it("fails clearly when both home and cwd are unavailable", () => {
    ...
    try {
      expect(() => resolveRequiredHomeDir({}, noHome)).toThrow(/set GRANTED_HOME/i);
...
describe("resolveOsHomeDir", () => {
  it("ignores GRANTED_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          GRANTED_HOME: "/srv/openclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { GRANTED_HOME: "/srv/openclaw-home" } as NodeJS.ProcessEnv,
      },
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of GRANTED_HOME", () => {
    expect(
