...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "GRANTED_LIVE_ANTHROPIC_KEY",
  "GRANTED_LIVE_ANTHROPIC_KEYS",
  "GRANTED_LIVE_GEMINI_KEY",
  "GRANTED_LIVE_OPENAI_KEY",
  "GRANTED_GATEWAY_TOKEN",
  "GRANTED_GATEWAY_PASSWORD",
  "GRANTED_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "GRANTED_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "GRANTED_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "GRANTED_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "GRANTED_BUNDLED_SKILLS_DIR=./attacker-skills",
...
describe("loadDotEnv", () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.openclaw/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        setTestEnvValue("GRANTED_STATE_DIR", defaultStateDir);
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "GRANTED_STATE_DIR=./evil-state",
            "GRANTED_CONFIG_PATH=./evil-config.json",
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "GRANTED_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "GRANTED_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "GRANTED_LIVE_GEMINI_KEY=sk-gemini-live",
            "GRANTED_LIVE_OPENAI_KEY=sk-openai-live",
            "GRANTED_GATEWAY_TOKEN=attacker-token",
            "GRANTED_GATEWAY_PASSWORD=attacker-password",
            "GRANTED_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
      ...
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          ...
          [
            ...
            "GRANTED_AGENT_DIR=./evil-agent",
            `GRANTED_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "GRANTED_OAUTH_DIR=./evil-oauth",
  ...
  it("blocks GRANTED_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "GRANTED_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.GRANTED_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.GRANTED_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "GRANTED_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'GRANTED_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.GRANTED_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.GRANTED_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.GRANTED_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.GRANTED_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "GRANTED_PINNED_PYTHON=./attacker-python",
            "GRANTED_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.GRANTED_PINNED_PYTHON;
        delete process.env.GRANTED_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.GRANTED_PINNED_PYTHON).toBeUndefined();
        expect(process.env.GRANTED_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "GRANTED_PINNED_PYTHON=/trusted/python",
            "GRANTED_PINNED_WRITE_PYTHON=/trusted/write-python",
        ...
        expect(process.env.GRANTED_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.GRANTED_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "GRANTED_LIVE_GEMINI_KEY=sk-gemini-live",
            "GRANTED_LIVE_OPENAI_KEY=sk-openai-live",
            "GRANTED_GATEWAY_TOKEN=trusted-token",
            "GRANTED_GATEWAY_PASSWORD=trusted-password",
            "GRANTED_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.GRANTED_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.GRANTED_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.GRANTED_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.GRANTED_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.GRANTED_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.GRANTED_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.GRANTED_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "GRANTED_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks GRANTED_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "GRANTED_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when GRANTED_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        setTestEnvValue("HOME", base);
        setTestEnvValue("GRANTED_STATE_DIR", customStateDir);
        await writeEnvFile(
          path.join(base, ".config", "openclaw", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.FOO).toBeUndefined();
        expect(process.env.GRANTED_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });
  ...
  it("blocks workspace .env takeover vars before loading the global fallback", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        expect(process.env.GRANTED_STATE_DIR).toBe(stateDir);
        expect(process.env.GRANTED_CONFIG_PATH).toBeUndefined();
        expect(process.env.GRANTED_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.NODE_OPTIONS).toBeUndefined();
        expect(process.env.NODE_REDIRECT_WARNINGS).toBeUndefined();
        expect(process.env.NODE_REPL_EXTERNAL_MODULE).toBeUndefined();
        expect(process.env.NODE_REPL_HISTORY).toBeUndefined();
