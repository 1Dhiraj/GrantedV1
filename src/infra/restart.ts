...
export function triggerOpenClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.GRANTED_SYSTEMD_UNIT,
      process.env.GRANTED_PROFILE,
    );
  ...
  const label =
    process.env.GRANTED_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.GRANTED_PROFILE);
