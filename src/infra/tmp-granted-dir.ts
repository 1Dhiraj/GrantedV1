// Creates temporary Granted directories for runtime scratch work.
import { getWorkerDeploySecureTempRoot } from "../worker/worker-deploy-runtime-registry.js";

/** Preferred shared Granted temp root on POSIX systems when ownership and permissions are safe. */
export const DEFAULT_POSIX_TMP_ROOT = "/tmp/granted";
...
export type ResolvePreferredGrantedTmpDirOptions = {
...
/** Resolves a safe Granted temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredGrantedTmpDir(
  options: ResolvePreferredGrantedTmpDirOptions = {},
): string {
  return loadResolveSecureTempRoot()({
    ...options,
    preferredDir: options.preferredDir ?? DEFAULT_POSIX_TMP_ROOT,
    fallbackPrefix: "granted",
    warningPrefix: "[granted]",
    unsafeFallbackLabel: "Granted temp dir",
