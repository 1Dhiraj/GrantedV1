// Covers preferred Granted temp directory resolution.
import { constants as fsConstants } from "node:fs";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { DEFAULT_POSIX_TMP_ROOT, resolvePreferredGrantedTmpDir } from "./tmp-granted-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredGrantedTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `granted-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredGrantedTmpDir({
    platform: "linux",
    accessSync,
    chmodSync,
...
describe("resolvePreferredGrantedTmpDir", () => {
  it("prefers /tmp/granted when it already exists and is writable", () => {
  ...
  it("honors a caller-selected secure root without changing the default temp policy", () => {
    const preferredDir = "/var/cache/granted";
  ...
  it("prefers /tmp/granted when it does not exist but /tmp is writable", () => {
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/granted when /tmp/granted is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/granted when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/granted exists but is not writable",
    ...
    {
      name: "falls back when /tmp/granted is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/granted is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/granted is group/other writable",
  ...
  it("repairs existing /tmp/granted permissions when they are too broad", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      `[granted] tightened permissions on temp dir: ${DEFAULT_POSIX_TMP_ROOT}`,
    );
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/granted after create when the initial mode stays too broad", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Granted temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "granted");
  ...
  it("repairs existing fallback directory when permissions are too broad", () => {
    ...
    expect(chmodSync).toHaveBeenCalledWith(fallbackPath, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[granted] tightened permissions on temp dir: ${fallbackPath}`,
    );
  });

  it("uses /tmp/granted when another process tightened permissions before repair", () => {
  ...
  it("uses /tmp/granted when chmod loses a concurrent repair race", () => {
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === DEFAULT_POSIX_TMP_ROOT && mode === 0o700) {
  ...
  it("throws when the fallback directory cannot be created", () => {
    ...
    ).toThrow(/Unable to create fallback Granted temp dir/);
  });
  ...
  it("skips the POSIX preferred path on Windows even when /tmp is accessible (#60713)", () => {
    ...
    // create it), the previous code path returned `/tmp/granted` and routed
    // log files / TTS temp files there instead of `%TEMP%\granted`. The
    // platform: "win32" branch must skip the POSIX path entirely.
    const winFallback = path.win32.join("C:\\Users\\u\\AppData\\Local\\Temp", "granted-501");
    ...
    const result = resolvePreferredGrantedTmpDir({
      platform: "win32",
      accessSync,
      lstatSync,
      mkdirSync,
