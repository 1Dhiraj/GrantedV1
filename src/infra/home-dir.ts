...
function resolveRawHomeDir(env: NodeJS.ProcessEnv, homedir: () => string): string | undefined {
  const explicitHome = normalize(env.GRANTED_HOME);
  if (!explicitHome) {
...
/** Resolves OpenClaw's effective home, honoring GRANTED_HOME before OS homes. */
...
/** Resolves the underlying OS user home, ignoring GRANTED_HOME overrides. */
...
export function resolveRequiredHomeDir(
  ...
  throw new Error(
    "Unable to resolve an OpenClaw home: set GRANTED_HOME, HOME, or USERPROFILE, or run from an existing directory.",
  );
}
...
/** Resolves a user-supplied path against the OS home, ignoring GRANTED_HOME. */
