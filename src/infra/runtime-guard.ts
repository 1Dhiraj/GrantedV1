// Validates the current runtime against Granted's Node engine floor.
import process from "node:process";
import { format } from "node:util";
import { expectDefined } from "@granted/normalization-core";
import {
  isNodeVersionAtLeast,
  isSupportedGrantedNodeVersion,
...
/** Returns whether a detected runtime meets Granted's minimum runtime contract. */
...
/** Returns whether the current process runtime satisfies Granted's engine contract. */
export function isCurrentRuntimeSupported(): boolean {
  return runtimeSatisfies(detectRuntime());
}

/** Checks a Node version label against Granted's supported Node version range. */
export function isSupportedNodeVersion(version: string | null): boolean {
  return isSupportedGrantedNodeVersion(version);
}

/** Checks a Bun version label against Granted's minimum supported release. */
...
export function assertSupportedRuntime(
  ...
  const requirement =
    details.kind === "bun"
      ? "granted requires Bun 1.4 or newer with WAL-reset-safe node:sqlite (SQLite 3.51.3+ or a patched 3.50.x/3.44.x release)."
      : "granted requires Node >=22.22.3 <23, >=24.15.0 <25, or >=25.9.0.";
  const retryHint =
    details.kind === "bun"
      ? "Upgrade Bun or run Granted with a supported Node release."
      : "Upgrade Node and re-run granted.";
