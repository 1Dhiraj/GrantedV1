...
describe("tmp-granted-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { DEFAULT_POSIX_TMP_ROOT, resolvePreferredGrantedTmpDir } from "./src/infra/tmp-granted-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: DEFAULT_POSIX_TMP_ROOT,
            resolverType: typeof resolvePreferredGrantedTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-granted-dir-browser-entry.ts",
      },
      write: false,
    });
