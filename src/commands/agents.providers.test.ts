...
describe("buildProviderStatusIndex", () => {
  ...
  it("prefers inspectAccount for read-only status surfaces", async () => {
    ...
    const map = await buildProviderStatusIndex({} as GrantedConfig);
  ...
  it("keeps unresolved configured SecretRef accounts visible without exposing their refs", async () => {
    ...
    await expect(buildProviderStatusIndex({} as GrantedConfig)).resolves.toEqual(
      new Map([
        [
          "quietchat:default",
          {
    ...
    const statuses = await buildProviderStatusIndex({} as GrantedConfig);
    expect(
      listProvidersForAgent({
        ...
        providerMetadata: buildProviderSummaryMetadataIndex({} as GrantedConfig),
      }),
  ...
  it("does not mark a healthy Slack account unavailable for an optional unresolved user token", async () => {
    ...
    expect(
      (await buildProviderStatusIndex({} as GrantedConfig)).get("slack:default"),
  ...
  it("does not inspect linkage for an unconfigured account", async () => {
    ...
    const status = (await buildProviderStatusIndex({} as GrantedConfig)).get("quietchat:default");
  ...
  it("uses the shipped custom state resolver when canonical linkage is unknown", async () => {
    ...
    const status = (await buildProviderStatusIndex({} as GrantedConfig)).get("legacychat:default");
  ...
  it("rethrows unexpected read-only account resolution errors", async () => {
    ...
    await expect(buildProviderStatusIndex({} as GrantedConfig)).rejects.toThrow("plugin crash");
  });
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHints.mockReturnValue([
      {
        ...
        pluginId: "feishu",
        label: "Feishu",
        installSpec: "@granted/feishu",
        installCommand: "openclaw plugins install @granted/feishu",
        doctorFixCommand: "openclaw doctor --fix",
        repairHint:
          "Install the official external plugin with: openclaw plugins install @granted/feishu, or run: openclaw doctor --fix.",
      },
    ]);
  ...
  it("uses repair hints instead of unknown for bound missing external channels", () => {
    ...
    expect(lines).toEqual([
      "Feishu default: missing plugin - Install the official external plugin with: openclaw plugins install @granted/feishu, or run: openclaw doctor --fix.",
    ]);
  });
