...
function hasPotentialPluginConfig(config: GrantedConfig, env: NodeJS.ProcessEnv): boolean {
...
function collectConfiguredMemoryEmbeddingProviderIds(config: GrantedConfig): ReadonlySet<string> {
...
function hasConfiguredCapabilityPlugin(config: GrantedConfig, env: NodeJS.ProcessEnv): boolean {
