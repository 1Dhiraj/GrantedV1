...
function materializeDefaultAgentRoles(cfg: GrantedConfig) {
...
function snapshotSurfaces(cfg: GrantedConfig): SurfaceSnapshot {
...
const fixtures: Array<{ name: string; config: GrantedConfig; materializes: boolean }> = [
...
describe("default agent role materialization", () => {
  ...
  it("keeps all-agent and per-agent heartbeat enrollment unchanged", () => {
    const allAgents: GrantedConfig = {
      agents: {
        defaults: { heartbeat: { every: "1h" } },
        entries: { ops: { default: true }, research: {} },
      },
    };
    const perAgent: GrantedConfig = {
      agents: {
        entries: {
          ops: { default: true },
