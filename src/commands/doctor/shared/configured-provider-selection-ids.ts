...
function collectConfiguredMediaProviderIds(cfg: GrantedConfig): Set<string> {
  const ids = new Set<string>();
  const add = (value: unknown) => {
    const id = normalizeId(value);
    if (id) {
...
export function collectConfiguredProviderSelectionIds(cfg: GrantedConfig): ReadonlySet<string> {
