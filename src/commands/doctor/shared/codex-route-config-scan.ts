...
}): { cfg: GrantedConfig; changes: string[] } {
...
function codexPluginIsBlockedOutsideEntry(cfg: GrantedConfig): boolean {
  return cfg.plugins?.enabled === false || pluginIdListIncludes(cfg.plugins?.deny, "codex");
}

export function codexPluginRepairIsBlocked(cfg: GrantedConfig): boolean {
  return (
...
function isCodexPluginUnavailableByConfig(cfg: GrantedConfig): boolean {
