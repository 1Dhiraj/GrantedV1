...
vi.mock("./shared.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveModelsTargetAgent: (_cfg: GrantedConfig, rawAgentId?: string) => ({
...
function applyCapturedConfigUpdate(cfg: GrantedConfig): GrantedConfig {
  const mutator = mocks.updateConfig.mock.calls[0]?.[0] as
    | ((current: GrantedConfig) => GrantedConfig)
    | undefined;
  if (!mutator) {
...
describe("models auth logout", () => {
  beforeEach(() => {
    ...
    mocks.updateConfig.mockResolvedValue({} as GrantedConfig);
    mocks.loadModelsConfig.mockResolvedValue({} as GrantedConfig);
