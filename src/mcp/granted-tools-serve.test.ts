...
describe("OpenClaw tools MCP server", () => {
  ...
  it("reads the managed bridge agent session key from env", () => {
    expect(
      resolveOpenClawToolsMcpAgentSessionKey({
        [GRANTED_TOOLS_MCP_AGENT_SESSION_KEY_ENV]: " agent:worker:main ",
      }),
  ...
  it("returns approved CLI MCP mutations to the host instead of applying them", async () => {
    ...
    vi.stubEnv(GRANTED_TOOLS_MCP_SYSTEM_AGENT_APPROVAL_ARMED_ENV, "1");
    vi.stubEnv(GRANTED_TOOLS_MCP_SYSTEM_AGENT_PROPOSAL_ENV, hashSystemAgentOperation(operation));
    const handlers = createPluginToolsMcpHandlers(
      resolveOpenClawToolsForMcp({ tools: ["openclaw"], systemAgentSurface: "cli" }),
    );
  ...
  it("parses the served tool selection from env and defaults to cron", () => {
    ...
    expect(
      resolveOpenClawToolsMcpToolSelection({
        [GRANTED_TOOLS_MCP_TOOLS_ENV]: " openclaw , cron ",
      }),
    ).toEqual(["openclaw", "cron"]);
    expect(() =>
      resolveOpenClawToolsMcpToolSelection({ [GRANTED_TOOLS_MCP_TOOLS_ENV]: "exec" }),
    ).toThrow(GRANTED_TOOLS_MCP_TOOLS_ENV);
  });
  ...
  it("parses the openclaw surface from env and defaults to cli", () => {
    ...
    expect(
      resolveOpenClawToolsMcpSystemAgentSurface({
        [GRANTED_TOOLS_MCP_SYSTEM_AGENT_SURFACE_ENV]: "gateway",
      }),
    ).toBe("gateway");
    expect(() =>
      resolveOpenClawToolsMcpSystemAgentSurface({
        [GRANTED_TOOLS_MCP_SYSTEM_AGENT_SURFACE_ENV]: "remote",
      }),
    ).toThrow(GRANTED_TOOLS_MCP_SYSTEM_AGENT_SURFACE_ENV);
  });
  ...
  it("builds a openclaw-only stdio server config under the openclaw name", () => {
    ...
    expect(server.args?.at(-1)).toMatch(/granted-tools-serve\.(js|ts)$/);
    expect(server.env).toEqual({
      [GRANTED_TOOLS_MCP_TOOLS_ENV]: "openclaw",
      [GRANTED_TOOLS_MCP_SYSTEM_AGENT_SURFACE_ENV]: "gateway",
    });
  });
  ...
  it("serializes operator-approval-only through the native CLI MCP config", () => {
    ...
    expect(server.env?.[GRANTED_TOOLS_MCP_SYSTEM_AGENT_APPROVAL_ARMED_ENV]).toBe("operator-only");
    ...
    expect(directServer.env).not.toHaveProperty(GRANTED_TOOLS_MCP_SYSTEM_AGENT_APPROVAL_ARMED_ENV);
    ...
    expect(armedServer.env?.[GRANTED_TOOLS_MCP_SYSTEM_AGENT_APPROVAL_ARMED_ENV]).toBe("1");
  });
  ...
  it("reconstructs the delegated refusal from env on the native CLI MCP tool", async () => {
    ...
    vi.stubEnv(GRANTED_TOOLS_MCP_SYSTEM_AGENT_APPROVAL_ARMED_ENV, "operator-only");
    vi.stubEnv(GRANTED_TOOLS_MCP_SYSTEM_AGENT_PROPOSAL_ENV, hashSystemAgentOperation(operation));
    const handlers = createPluginToolsMcpHandlers(
      resolveOpenClawToolsForMcp({ tools: ["openclaw"], systemAgentSurface: "cli" }),
    );
