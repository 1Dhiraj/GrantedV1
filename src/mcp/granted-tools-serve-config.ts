/**
 * Shared contract between the granted-tools MCP stdio entry and the callers
...
function resolveOpenClawToolsServeCommand(): { command: string; args: string[] } {
  ...
  if (!packageRoot) {
    throw new Error("granted-tools MCP: could not resolve the OpenClaw package root");
  }
  const distEntry = path.join(packageRoot, "dist", "mcp", "granted-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return { command: process.execPath, args: [distEntry] };
  }
  const sourceEntry = path.join(packageRoot, "src", "mcp", "granted-tools-serve.ts");
  if (!fs.existsSync(sourceEntry)) {
    throw new Error(`granted-tools MCP: no serve entry under ${packageRoot}`);
  }
