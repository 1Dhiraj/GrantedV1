...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "GRANTED_ENV_TEST_A";
    const keyB = "GRANTED_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "GRANTED_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "GRANTED_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "GRANTED_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "GRANTED_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "GRANTED_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "GRANTED_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "GRANTED_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    setTestEnvValue(key, "outer");
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    ...
    const previousBundledDir = process.env.GRANTED_BUNDLED_PLUGINS_DIR;
    setTestEnvValue("GRANTED_HOME", "/srv/openclaw-home");
    setTestEnvValue("GRANTED_STATE_DIR", "/srv/openclaw-state");
    setTestEnvValue("GRANTED_BUNDLED_PLUGINS_DIR", "/srv/openclaw-bundled");
    ...
    try {
      ...
      expect(env.GRANTED_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.GRANTED_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("GRANTED_HOME", previousOpenClawHome);
      restoreEnvKey("GRANTED_STATE_DIR", previousStateDir);
      restoreEnvKey("GRANTED_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processOpenClawHome: process.env.GRANTED_HOME,
          processBundledDir: process.env.GRANTED_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.GRANTED_BUNDLED_PLUGINS_DIR,
        }),
      );
      ...
      expect(process.env.GRANTED_HOME).toBe("/srv/openclaw-home");
    } finally {
      restoreEnvKey("GRANTED_HOME", previousOpenClawHome);
    }
  });
