...
describe("createTempHomeEnv", () => {
  it.each(["directory", "environment"])(
    ...
    async (stage) => {
      ...
      try {
        await withEnvAsync({ USERPROFILE: undefined, GRANTED_STATE_DIR: "" }, async () => {
          const keys = ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "GRANTED_STATE_DIR"];
  ...
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    expect(process.env.GRANTED_STATE_DIR).toBe(path.join(tempHome.home, ".openclaw"));
...
describe("withTempHome acquisition", () => {
  ...
  it.each([false, true])(
    ...
    async (skipHomeCleanup) => {
      ...
      await withEnvAsync(
        {
          GRANTED_HOME: callerHome,
          GRANTED_STATE_DIR: path.join(callerHome, ".openclaw"),
