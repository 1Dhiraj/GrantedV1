 ...
 */
export function isAuditLedgerEnabled(cfg: GrantedConfig | undefined): boolean {
  return cfg?.logging?.audit?.enabled !== false;
}

/** Execution identity is retained only after an explicit startup-scoped opt-in. */
export function isExecutionIdentityCollectionEnabled(cfg: GrantedConfig | undefined): boolean {
...
export function resolveAuditMessageMode(cfg: GrantedConfig | undefined): AuditMessageMode {
