...
export function hasSameSecretReloadContract(left: GrantedConfig, right: GrantedConfig): boolean {
...
export function hasSameSecretProviderDefinition(ref: SecretRef, configs: GrantedConfig[]): boolean {
  ...
  const dependency = (config: GrantedConfig) => ({
...
function preserveResolvedSecretRefValues(
  ...
  restored: unknown,
  sourceConfig: GrantedConfig,
  currentSourceConfig: GrantedConfig,
): unknown {
...
function mergeRollbackAuthStoreCredentials(
  ...
  current: Record<string, AuthProfileStore>,
  restored: Record<string, AuthProfileStore>,
  configs: [GrantedConfig, GrantedConfig, GrantedConfig],
  mutationLineage: typeof activeSnapshotLineageAuthMutations,
...
export function setSecretsRuntimeSourceSnapshotIfCurrent(params: {
  ...
  runtimeSourceConfig: GrantedConfig;
  secretsSourceConfig: GrantedConfig;
}): boolean {
...
function advanceSecretsRuntimeSourceSnapshot(sourceConfig: GrantedConfig): void {
  if (activeSnapshot) {
