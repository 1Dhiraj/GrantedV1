...
function toCandidate(
  ...
  defaults: NonNullable<GrantedConfig["secrets"]>["defaults"] | undefined,
 ...
 */
export function migrateLegacySecretRefEnvMarkers(config: GrantedConfig): {
  config: GrantedConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as GrantedConfig & Record<string, unknown>;
