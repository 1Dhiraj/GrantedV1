...
} from "./granted-database-verify.impl.js";
import {
  type GrantedDatabaseVerifyResult,
  type GrantedDatabaseVerifyTarget,
  verifyOpenClawDatabases,
} from "./granted-database-verify.worker.js";
import {
  clearOpenClawDatabaseQuarantine,
  readOpenClawDatabaseQuarantine,
  recordOpenClawDatabaseQuarantine,
} from "./granted-quarantine-store.js";
import {
...
describe("OpenClaw database integrity verifier", () => {
  it.each(["absent", "installed"])(
    ...
    async (shape) => {
      const stateDir = tempDirs.make("granted-database-verify-eligibility-");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("granted-database-verify-live-locks-");
      ...
      const reader = spawnSync(
        ...
        [
          ...
          `
            ...
            const database = new DatabaseSync(process.env.GRANTED_VERIFY_TEST_PATH);
            database.prepare("PRAGMA schema_version;").get();
            database.close();
          `,
        ],
        {
          env: { ...process.env, GRANTED_VERIFY_TEST_PATH: agent.path },
          encoding: "utf8",
        },
      );
  ...
  it("detects corruption off-thread, quarantines it, and latches later opens", async () => {
    const stateDir = tempDirs.make("granted-database-verify-");
    const env = { GRANTED_STATE_DIR: stateDir };
  ...
  it("does not quarantine a healthy database that replaced the verified file", async () => {
    const stateDir = tempDirs.make("granted-database-verify-replacement-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const agentPath = openOpenClawAgentDatabase({ agentId: "worker-1", env }).path;
    closeOpenClawAgentDatabasesForTest();
    closeOpenClawStateDatabaseForTest();
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("granted-database-verify-live-agent-replace-");
      const env = { GRANTED_STATE_DIR: stateDir };
      const agent = openOpenClawAgentDatabase({ agentId: "worker-1", env });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const stateDir = tempDirs.make("granted-database-verify-live-state-replace-");
      const env = { GRANTED_STATE_DIR: stateDir };
  ...
  it("reconfirms and quarantines a corrupt closed database", async () => {
    const stateDir = tempDirs.make("granted-database-verify-closed-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const agentPath = openOpenClawAgentDatabase({ agentId: "worker-1", env }).path;
    closeOpenClawAgentDatabasesForTest();
    closeOpenClawStateDatabaseForTest();
  ...
  it("does not quarantine a repaired database after same-inode mutation", async () => {
    const stateDir = tempDirs.make("granted-database-verify-repair-race-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const agentPath = openOpenClawAgentDatabase({ agentId: "worker-1", env }).path;
    closeOpenClawAgentDatabasesForTest();
    closeOpenClawStateDatabaseForTest();
  ...
  it("rejects stale terminal results after draining healthy owners", () => {
    const stateDir = tempDirs.make("granted-database-verify-live-stale-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const state = openOpenClawStateDatabase({ env });
    const agent = openOpenClawAgentDatabase({ agentId: "worker-1", env });
    const targets: GrantedDatabaseVerifyTarget[] = [
  ...
  it("revalidates agent schema ownership after confirmation drains a pathname", () => {
    const stateDir = tempDirs.make("granted-database-verify-agent-revalidate-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const agent = openOpenClawAgentDatabase({ agentId: "worker-1", env });
  ...
  it("expires a generation-bound process latch after the database changes", () => {
    const stateDir = tempDirs.make("granted-database-verify-latch-generation-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const agentPath = openOpenClawAgentDatabase({ agentId: "worker-1", env }).path;
    closeOpenClawAgentDatabasesForTest();
  ...
  it("reports an uncleared quarantine row instead of claiming repair success", () => {
    const stateDir = tempDirs.make("granted-database-verify-clear-failure-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const agentPath = openOpenClawAgentDatabase({ agentId: "worker-1", env }).path;
  ...
  it("keeps healthy opens on the missing-store fast path", () => {
    const stateDir = tempDirs.make("granted-database-verify-clean-");
    const env = { GRANTED_STATE_DIR: stateDir };
  ...
  it("records and clears dedicated quarantine rows with rollback journaling", () => {
    const stateDir = tempDirs.make("granted-database-verify-store-");
    const env = { GRANTED_STATE_DIR: stateDir };
  ...
  it("expires a persisted quarantine when the verified database generation changes", () => {
    const stateDir = tempDirs.make("granted-database-verify-store-generation-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const { DatabaseSync } = requireNodeSqlite();
    const database = new DatabaseSync(databasePath);
  ...
  it("reads schema-v1 quarantine rows and migrates them on the next write", () => {
    const stateDir = tempDirs.make("granted-database-verify-store-v1-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const storePath = quarantineStorePath(stateDir);
  ...
  it("recovers an interrupted empty quarantine-store initialization", () => {
    const stateDir = tempDirs.make("granted-database-verify-empty-store-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agent.sqlite");
    const storePath = quarantineStorePath(stateDir);
    fs.mkdirSync(path.dirname(storePath), { recursive: true });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      const stateDir = tempDirs.make("granted-database-verify-hot-journal-");
      const env = { GRANTED_STATE_DIR: stateDir };
      const databasePath = path.join(stateDir, "agent.sqlite");
      const storePath = quarantineStorePath(stateDir);
      expect(
      ...
      const crashed = spawnSync(
        ...
        [
          ...
          `
            import { DatabaseSync } from "node:sqlite";
            const database = new DatabaseSync(process.env.GRANTED_QUARANTINE_TEST_PATH);
        ...
        { env: { ...process.env, GRANTED_QUARANTINE_TEST_PATH: storePath } },
      );
  ...
  it("does not latch transient verifier errors", () => {
    const stateDir = tempDirs.make("granted-database-verify-transient-");
    const env = { GRANTED_STATE_DIR: stateDir };
    const agentPath = openOpenClawAgentDatabase({ agentId: "worker-1", env }).path;
    closeOpenClawAgentDatabasesForTest();
    closeOpenClawStateDatabaseForTest();
  ...
  it("persists state failure quarantine across restart until doctor repair", () => {
    const stateDir = tempDirs.make("granted-database-verify-state-failure-");
    const env = { GRANTED_STATE_DIR: stateDir };
