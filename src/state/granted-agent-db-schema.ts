...
import { ensureOpenClawAgentBoardSchemaInTransaction } from "./granted-agent-board-schema.js";
import {
  AGENT_MEDIA_SCHEMA_VERSION,
  GRANTED_AGENT_SCHEMA_VERSION,
  type GrantedAgentDatabaseOptions,
} from "./granted-agent-db-contract.js";
import * as maintenanceAuthority from "./granted-agent-db-lease.js";
import { ensureOpenClawAgentDatabasePermissions } from "./granted-agent-db-permissions.js";
import { registerOpenClawAgentDatabase } from "./granted-agent-db-registry.js";
import {
...
} from "./granted-agent-db-session-migrations.js";
import { migrateSessionNodesAndWindows } from "./granted-agent-db-session-nodes-migration.js";
import {
  addSessionProvenanceColumns,
  backfillSessionEntryProvenance,
  backfillTranscriptMutationWatermarks,
} from "./granted-agent-db-session-provenance.js";
import type { DB as GrantedAgentKyselyDatabase } from "./granted-agent-db.generated.js";
import { resolveOpenClawAgentSqlitePath } from "./granted-agent-db.paths.js";
import {
  migrateSessionParticipantsSchema,
  withLegacySessionParticipantsSchema,
} from "./granted-agent-participants-migration.js";
import { hasPendingInputConsumptionColumnMigration } from "./granted-agent-pending-inputs-schema.js";
import { GRANTED_AGENT_SCHEMA_SQL } from "./granted-agent-schema.js";
import { GRANTED_SQLITE_BUSY_TIMEOUT_MS } from "./granted-state-db.js";
...
export function assertAgentDatabaseIntegrityBeforeMutation(
  ...
  if (userVersion === GRANTED_AGENT_SCHEMA_VERSION && !hasPendingCurrentVersionMigration) {
    verifyAndRepairCanonicalSqliteIndexes(database, pathname, GRANTED_AGENT_SCHEMA_SQL, {
...
function ensureAgentSchema(
  ...
  agentId: string,
  pathname: string,
  targetVersion = GRANTED_AGENT_SCHEMA_VERSION,
): void {
  const schemaSql =
    targetVersion < 18
  ...
  try {
    runSqliteImmediateTransactionSync(db, () => {
      ...
      if (previousVersion === AGENT_MEDIA_SCHEMA_VERSION) {
        const legacySql = withLegacySessionParticipantsSchema(GRANTED_AGENT_SCHEMA_SQL);
