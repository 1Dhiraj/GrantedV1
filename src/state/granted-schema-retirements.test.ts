...
import retirementManifest from "./granted-schema-retirements.json" with { type: "json" };
...
it("keeps the schema retirement ledger aligned with canonical database schemas", () => {
  ...
  const databases = {
    state: {
      ...
      schemaUrl: new URL("./granted-state-schema.sql", import.meta.url),
      database: new DatabaseSync(":memory:"),
    },
    agent: {
      currentVersion: GRANTED_AGENT_SCHEMA_VERSION,
      schemaUrl: new URL("./granted-agent-schema.sql", import.meta.url),
      database: new DatabaseSync(":memory:"),
    },
  } satisfies Record<
    DatabaseOwner,
