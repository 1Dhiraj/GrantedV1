...
import { runDatabaseVerifyWorker } from "./granted-database-verify.impl.js";

const tempDirs = useAutoCleanupTempDirTracker(afterEach);

async function importVerifierInUnrelatedFork(): Promise<unknown[]> {
  const fixtureDir = tempDirs.make("granted-database-verify-process-");
...
describe("database verifier child process entrypoint", () => {
  it("preserves inherited Node flags for a JavaScript fork", async () => {
    const fixtureDir = tempDirs.make("granted-database-verify-flags-");
