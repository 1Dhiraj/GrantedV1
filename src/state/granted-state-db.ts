...
import { clearOpenClawDatabaseQuarantine } from "./granted-quarantine-store.js";
import { repairAuditEventsSchema } from "./granted-state-db-audit-migration.js";
import {
  openClawStateDatabaseCache as stateDbCache,
  recordOpenClawStateDatabaseOpenFailure,
  clearOpenClawStateDatabaseOpenFailure,
  closeOpenClawStateDatabaseByPath,
} from "./granted-state-db-cache.js";
import {
...
} from "./granted-state-db-fast-path.js";
import {
  ...
  resolveDatabasePath,
} from "./granted-state-db-maintenance.js";
import { openUnpublishedStateDatabase } from "./granted-state-db-open.js";
import * as operatorApprovalMigration from "./granted-state-db-operator-approval-migration.js";
import { ensureOpenClawStatePermissions } from "./granted-state-db-permissions.js";
import {
  ensureAdditiveStateColumns,
  ensureFirstUseAdditiveStateColumnsForStrictMigration,
} from "./granted-state-db-schema-additive.js";
...
} from "./granted-state-db-schema-repair.js";
import { migrateSingletonStateFoldInV12 } from "./granted-state-db-schema-v12-foldin.js";
import { migrateJsonCanonicalWideRowsV13 } from "./granted-state-db-schema-v13-widerow.js";
import * as sessionWatchMigration from "./granted-state-db-session-watch-migration.js";
import {
  initializeNativeOpenClawStateConnection,
  isUninitializedNativeStartupDatabase,
  withOpenClawStateStartupCheckpointConnection,
} from "./granted-state-db-startup-checkpoint.js";
import * as retirements from "./granted-state-db-table-retirements.js";
import type { DB as GrantedStateKyselyDatabase } from "./granted-state-db.generated.js";
import { describeAgentPathMigration, warnAgentPathMigration } from "./granted-state-db.paths.js";
import {
  assertOpenClawStateWriteAllowed,
  isOpenClawStateWriteContentionError,
  GrantedStateOwnershipError,
  runWithOpenClawStateWriteAccess,
} from "./granted-state-ownership.js";
...
export { registerOpenClawStateDatabaseLifecycleListener } from "./granted-state-db-cache.js";
...
export { ensureOpenClawStatePermissions } from "./granted-state-db-permissions.js";
export { detectOpenClawStateDatabaseSchemaMigrations } from "./granted-state-db-schema-repair.js";
  ...
  try {
    ...
    const changes = runSqliteImmediateTransactionSync(
      ...
      () => {
        ...
        if (previousVersion === GRANTED_STATE_SCHEMA_VERSION) {
          for (const name of repairCanonicalSqliteIndexes(db, pathname, GRANTED_STATE_SCHEMA_SQL, {
          ...
          assertSqliteSchemaTablesPresent(db, pathname, GRANTED_STATE_SCHEMA_SQL, {
        ...
        if (tableExists(db, "audit_events")) {
          ...
          executeCanonicalStateSchema(db, {
            includeVersionLazyAdditiveTables: previousVersion !== GRANTED_STATE_SCHEMA_VERSION,
          });
          if (previousVersion < GRANTED_STATE_STRICT_SCHEMA_VERSION) {
        ...
        markCurrentStateSchemaVersion(db, {
          createMetadataIfMissing: previousVersion < GRANTED_STATE_SCHEMA_VERSION,
        });
        if (readSqliteUserVersion(db) === GRANTED_STATE_SCHEMA_VERSION) {
...
function needsOpenClawStateDatabaseSchemaRepair(pathname: string): boolean {
  ...
  try {
    ...
    const needsRepair =
      readSqliteUserVersion(database) !== GRANTED_STATE_SCHEMA_VERSION ||
    ...
    try {
      runSqliteImmediateTransactionSync(
        ...
        () => {
          ...
          const previousVersion = readSqliteUserVersion(db);
          if (previousVersion === GRANTED_STATE_SCHEMA_VERSION) {
            verifyAndRepairCanonicalSqliteIndexes(db, pathname, GRANTED_STATE_SCHEMA_SQL, {
              allowMissingColumns: true,
          ...
          repairCanonicalSqliteIndexes(db, pathname, GRANTED_STATE_SCHEMA_SQL, {
            verifyPhysicalIntegrity: false,
          });
            ...
            kysely
              ...
              .onConflict((conflict) =>
                conflict
                  ...
                  .where((eb) =>
                    eb.or([
                      eb("schema_meta.schema_version", "!=", GRANTED_STATE_SCHEMA_VERSION),
...
export function runWithOpenClawStateBusyTimeout<T>(
  ...
  try {
    ...
    if (opened.db.isOpen) {
      setSqliteBusyTimeout(opened.db, GRANTED_SQLITE_BUSY_TIMEOUT_MS);
    }
  }
}
