...
describe("media store", () => {
  ...
  it("prefers header mime extension when sniffed mime lacks mapping", async () => {
    await withTempStore(async (_store, homeLocal) => {
      vi.doMock("@granted/media-core/mime", async () => {
        const actual = await vi.importActual<typeof import("@granted/media-core/mime")>(
          "@granted/media-core/mime",
        );
        return {
          ...actual,
      ...
      try {
        ...
        vi.doUnmock("@granted/media-core/mime");
      }
    });
  });
