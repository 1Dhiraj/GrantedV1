...
type GrantedSkillSecurityVerdictItem = Omit<
...
function projectClawHubVerdictItem(
...
): GrantedSkillSecurityVerdictItem {
  const projected: GrantedSkillSecurityVerdictItem = {
  ...
  if (item.error) {
    const error: GrantedSkillSecurityVerdictItem["error"] = {};
...
export async function fetchOpenClawSkillSecurityVerdicts(
...
): Promise<GrantedSkillSecurityVerdictItem[]> {
  ...
  const items: GrantedSkillSecurityVerdictItem[] = [];
