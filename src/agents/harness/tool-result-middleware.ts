...
type MiddlewareContentBlock = GrantedAgentToolResult["content"][number];
...
function isValidMiddlewareToolResult(value: unknown): value is GrantedAgentToolResult {
...
function coerceMiddlewareToolResult(
  value: unknown,
  options: MiddlewareToolResultCoerceOptions = {},
): GrantedAgentToolResult | undefined {
  ...
  const content: GrantedAgentToolResult["content"] = [];
 ...
 */
function sanitizeToolResultForMiddleware(result: GrantedAgentToolResult): GrantedAgentToolResult {
...
function buildMiddlewareFailureResult(): GrantedAgentToolResult {
  return {
    content: [
      {
        type: "text",
...
function reconcileDeliveredMessagingFailure(
  result: GrantedAgentToolResult,
  fallback: GrantedAgentToolResult | undefined,
): GrantedAgentToolResult {
...
export function createAgentToolResultMiddlewareRunner(
  ...
  return {
    async applyToolResultMiddleware(
      event: AgentToolResultMiddlewareEvent,
    ): Promise<GrantedAgentToolResult> {
