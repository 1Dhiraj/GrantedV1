...
import { filterToolsByClientCaps } from "./granted-tools.client-caps.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./granted-tools.media-factory-plan.js";
import { createMediaGenerationAsyncStartCallback } from "./granted-tools.media-yield.js";
import { applyNodesToolWorkspaceGuard } from "./granted-tools.nodes-workspace-guard.js";
import {
...
} from "./granted-tools.registration.js";
import { createRequesterYieldCallback } from "./granted-tools.requester-yield.js";
import { createOpenClawSwarmToolGroups } from "./granted-tools.swarm.js";
import { resolveTranscriptsTool } from "./granted-tools.transcripts.js";
import type { GrantedToolsOptions } from "./granted-tools.types.js";
import { resolveWidgetPresentationForRun } from "./granted-tools.widget-presentation.js";
...
export { filterToolsByClientCaps } from "./granted-tools.client-caps.js";
export function createOpenClawTools(options?: GrantedToolsOptions): AnyAgentTool[] {
  ...
  options?.recordToolPrepStage?.("granted-tools:session-workspace");
  ...
  const imageTool =
    ...
    resolveImageToolFactoryAvailable({
      ...
      : null;
  options?.recordToolPrepStage?.("granted-tools:image-tool");
  ...
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("granted-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("granted-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("granted-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("granted-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("granted-tools:nodes-tool");
  ...
  options?.recordToolPrepStage?.("granted-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("granted-tools:plugin-tools");
  }

  allTools = filterToolsByClientCaps(allTools, options?.clientCaps);
  options?.recordToolPrepStage?.("granted-tools:client-capabilities");
  ...
  options?.recordToolPrepStage?.("granted-tools:tool-hooks");
  return allTools
    .map((tool) =>
