...
} from "./granted-tools.subagents.sessions-spawn.test-harness.js";
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.GRANTED_TEST_FAST;
...
describe("granted-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(async () => {
  ...
  afterAll(() => {
    ...
    process.env.GRANTED_TEST_FAST = fastModeEnv.previous;
  });
