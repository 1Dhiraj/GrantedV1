...
abstract class GrantedMcpHttpTransport implements Transport {
  onclose?: () => void;
...
export class GrantedSSEClientTransport extends GrantedMcpHttpTransport {
...
type GrantedStreamableHttpOptions = StreamableHTTPClientTransportOptions & {
  fetch?: FetchLike;
  requestInit?: RequestInit;
};

/** Owns Streamable HTTP notification recovery and stateful cleanup around SDK 1.30.0. */
export class GrantedStreamableHTTPClientTransport extends GrantedMcpHttpTransport {
  ...
  constructor(url: URL, options: GrantedStreamableHttpOptions = {}) {
    super();
    this.url = url;
