...
describe("runExecProcess POSIX command wrapper", () => {
  ...
  it("wraps command with PATH export if GRANTED_PREPEND_PATH is present", async () => {
    ...
    expect(spawnCall.argv.join(" ")).toContain(
      'export PATH="${GRANTED_PREPEND_PATH}${PATH:+:$PATH}"; unset GRANTED_PREPEND_PATH; echo test',
    );
  });
