...
type CreateOpenClawToolsOptions = Parameters<
  typeof import("./granted-tools.js").createOpenClawTools
>[0];
let createOpenClawToolsForTestModule: typeof import("./granted-tools.js").createOpenClawTools;
...
function createExplicitMediaModelConfig(): GrantedConfig {
  return {
    agents: {
      defaults: {
        mediaModels: {
...
function legacyModelProviderConfig(provider: Record<string, unknown>): GrantedConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: GrantedConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
