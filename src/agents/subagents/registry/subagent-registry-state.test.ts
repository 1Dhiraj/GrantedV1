...
describe("subagent registry state read cache", () => {
  const previousReadSqliteFlag = process.env.GRANTED_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE;
  ...
  afterEach(() => {
    ...
    if (previousReadSqliteFlag === undefined) {
      ...
      process.env.GRANTED_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE = previousReadSqliteFlag;
    }
    vi.useRealTimers();
  });
