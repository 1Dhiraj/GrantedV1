...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): GrantedConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): GrantedConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): GrantedConfig =>
  ({
    agents: {
      list: [{ id: "test" }],
    },
