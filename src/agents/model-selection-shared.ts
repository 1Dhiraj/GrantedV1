...
export { resolvePrimaryStringValue as normalizeModelSelection } from "@granted/normalization-core/string-coerce";
...
function listConfiguredModelMaps(cfg: GrantedConfig, agentId?: string) {
  return [
...
export function listModelAliasCandidates(cfg: GrantedConfig, agentId?: string) {
...
function hasConfiguredProviderModelRows(cfg: GrantedConfig): boolean {
...
function hasConfiguredProviderRowsNeedingManifestLookup(cfg: GrantedConfig): boolean {
  const providers = cfg.models?.providers;
  if (!providers || typeof providers !== "object") {
    return false;
  }
