...
const ENV_KEYS = ["GRANTED_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL", "SSLKEYLOGFILE"] as const;
...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("GRANTED_SHELL")) {
        input.onStdout?.(env.GRANTED_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  it("sets GRANTED_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }
    ...
    const result = await tool.execute("call-openclaw-shell", {
      command: 'printf "%s" "${GRANTED_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
    const value = normalizeText(result.content.find((c) => c.type === "text")?.text);
...
describe("exec host env validation", () => {
  ...
  it.each([
    ...
    "sudo GRANTED_APPROVE=1 /approve abc123 allow-once",
    "sudo -uroot bash -lc '/approve abc123 allow-once'",
    "sudo -u root GRANTED_APPROVE=1 bash -lc '/approve abc123 allow-once'",
