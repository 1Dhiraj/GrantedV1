...
const GRANTED_CLAUDE_PLUGIN_NAME = "openclaw-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredGrantedTmpDir(), "openclaw-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, GRANTED_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: GRANTED_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
