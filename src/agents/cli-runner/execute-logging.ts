...
export const CLI_BACKEND_PRESERVE_ENV = "GRANTED_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.GRANTED_MCP_TOKEN ? "set" : "missing"}`,
    `capture=${childEnv.GRANTED_MCP_CLI_CAPTURE_KEY ? "set" : "missing"}`,
  ].join(" ");
}
...
export function logCliInvocation(params: {
  ...
  if (params.env.GRANTED_MCP_TOKEN) {
