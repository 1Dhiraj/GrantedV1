...
function acpBinding(params: {
  ...
  agentId?: string;
  cwd?: string;
}): NonNullable<GrantedConfig["bindings"]>[number] {
  return {
    type: "acp",
  ...
  } as NonNullable<GrantedConfig["bindings"]>[number];
}
...
async function bindingSessionKey(cfg: GrantedConfig, conversationId: string): Promise<string> {
