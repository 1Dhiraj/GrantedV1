...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "GRANTED_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { GRANTED_LIVE_MODEL_IMAGE_PROBE: "false" },
        "GRANTED_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { GRANTED_LIVE_MODEL_IMAGE_PROBE: "1" },
        "GRANTED_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
