...
describe("readStateDirDotEnvFromStateDir", () => {
  ...
  it("skips values that are unresolved shell variable references", async () => {
    const content = [
      'SUPERMEMORY_GRANTED_API_KEY="${SUPERMEMORY_GRANTED_KEY}"',
      "QUOTED_SUPERMEMORY_GRANTED_API_KEY='\"$SUPERMEMORY_GRANTED_KEY\"'",
    ...
    await withDotEnv(content, async (dir) => {
      const result = readStateDirDotEnvFromStateDir(dir).entries;
      expect(Object.keys(result)).not.toContain("SUPERMEMORY_GRANTED_API_KEY");
      expect(Object.keys(result)).not.toContain("QUOTED_SUPERMEMORY_GRANTED_API_KEY");
