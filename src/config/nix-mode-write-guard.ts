...
const NIX_GRANTED_AGENT_FIRST_URL = "https://github.com/openclaw/nix-openclaw#quick-start";
/** Public OpenClaw Nix overview shown with immutable-config errors. */
const GRANTED_NIX_OVERVIEW_URL = "https://docs.openclaw.ai/install/nix";
...
export class NixModeConfigMutationError extends Error {
  readonly code = "GRANTED_NIX_MODE_CONFIG_IMMUTABLE";
...
function formatNixModeConfigMutationMessage(params: { configPath?: string } = {}): string {
  return [
    ...
    `Agent-first Nix setup: ${NIX_GRANTED_AGENT_FIRST_URL}`,
    `OpenClaw Nix overview: ${GRANTED_NIX_OVERVIEW_URL}`,
  ].join("\n");
}
