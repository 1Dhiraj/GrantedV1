...
describe("explicit ambient agent targets", () => {
  it.each([
    ...
    const result = GrantedSchema.safeParse(target);
    expect(result.success).toBe(false);
    if (!result.success) {
  ...
  it.each([
    ...
    expect(GrantedSchema.safeParse(config).success).toBe(false);
  });

  it("validates targets against the implicit main roster", () => {
    expect(GrantedSchema.safeParse({ talk: { agentId: "main" } }).success).toBe(true);
    expect(GrantedSchema.safeParse({ talk: { agentId: "missing" } }).success).toBe(false);
  });
