...
describe("GrantedSchema talk validation", () => {
  ...
  it.each([
    ...
    expect(() => GrantedSchema.parse({ talk: { realtime } })).toThrow();
  });
  ...
  it.each(["constructor", "__proto__"])(
    ...
    (provider) => {
      ...
      expect(GrantedSchema.safeParse({ talk: { provider, providers } }).success).toBe(false);
      expect(GrantedSchema.safeParse({ talk: { realtime: { provider, providers } } }).success).toBe(
        false,
      );
    },
  );
