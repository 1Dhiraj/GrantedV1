...
export const AGENT_FIELD_HELP: Record<string, string> = {
  ...
  "agents.defaults.economyModel":
    "Optional cheap model (provider/model or alias) for routine work. Background turns fall back to it for heartbeats, spawned sub-agents, isolated cron runs, and compaction; the bundled task router also uses it for ordinary chat, trivial replies, and ambiguous-task classification while keeping browser and desktop automation on the primary model. Explicit per-role and task-router model settings still win. Unlike utilityModel, which handles short internal completions such as titles, this can run complete agent turns; leave unset to keep routine work on the primary model.",
  ...
  "agents.defaults.spendLimitUsdByProvider":
    'Per-provider cumulative USD ceilings keyed by provider id (for example {"anthropic": 20}). When one provider hits its ceiling only that provider is blocked, and model fallback moves on to the next configured provider; the global spendLimitUsd still applies on top.',
