...
function isBlockedConfigEnvVar(key: string): boolean {
  return (
    ...
    key.toUpperCase() === "GRANTED_INCLUDE_ROOTS" ||
...
function collectConfigEnvVarsByTarget(cfg?: GrantedConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: GrantedConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** Collects config env vars safe to persist into managed service environments. */
export function collectConfigServiceEnvVars(cfg?: GrantedConfig): Record<string, string> {
...
type PublishedConfigRuntimeEnvState = {
  ...
  sourceConfig: GrantedConfig | null;
};
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.GRANTED_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
