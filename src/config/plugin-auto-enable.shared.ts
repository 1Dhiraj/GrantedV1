...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: GrantedConfig): boolean {
...
function hasConfiguredEmbeddedHarnessRuntime(cfg: GrantedConfig, _env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: GrantedConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: GrantedConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: GrantedConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: GrantedConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: GrantedConfig): boolean {
...
function hasConfiguredVoiceProviderSelection(cfg: GrantedConfig): boolean {
  return Boolean(
...
function hasBrowserToolReference(cfg: GrantedConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: GrantedConfig): string[] {
  const entries = asOptionalObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function hasOwnPluginEntry(cfg: GrantedConfig, pluginId: string): boolean {
  const entries = asOptionalObjectRecord(cfg.plugins?.entries);
  return entries !== undefined && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: GrantedConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: GrantedConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: GrantedConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: GrantedConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: GrantedConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: GrantedConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: GrantedConfig): boolean {
  return (
...
function hasPluginEntries(cfg: GrantedConfig): boolean {
  const entries = asOptionalObjectRecord(cfg.plugins?.entries);
  return entries !== undefined && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: GrantedConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: GrantedConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: GrantedConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: GrantedConfig, env: NodeJS.ProcessEnv): boolean {
...
export function configMayNeedPluginAutoEnable(cfg: GrantedConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: GrantedConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: GrantedConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: GrantedConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: GrantedConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
