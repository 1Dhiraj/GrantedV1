...
let runtimeConfigSnapshot: GrantedConfig | null = null;
let runtimeConfigSourceSnapshot: GrantedConfig | null = null;
...
const runtimeConfigSnapshotPreparers = new Set<(config: GrantedConfig) => void>();
...
function configSnapshotsMatch(left: GrantedConfig, right: GrantedConfig): boolean {
  if (left === right) {
    return true;
  }
...
export function hashRuntimeConfigValue(value: GrantedConfig): string {
...
export function registerRuntimeConfigSnapshotPreparer(
  prepare: (config: GrantedConfig) => void,
): () => void {
...
export function getRuntimeConfigSnapshot(): GrantedConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): GrantedConfig | null {
...
export function resolveRuntimeConfigCacheKey(config: GrantedConfig): string {
...
export function selectApplicableRuntimeConfig(params: {
  ...
  runtimeConfig?: GrantedConfig | null;
  runtimeSourceConfig?: GrantedConfig | null;
...
export function loadPinnedRuntimeConfig(loadFresh: () => GrantedConfig): GrantedConfig {
  if (runtimeConfigSnapshot) {
    return runtimeConfigSnapshot;
  }
