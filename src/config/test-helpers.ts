...
import { resetConfigRuntimeState, type GrantedConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        GRANTED_MPM_CATALOG_PATHS: undefined,
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.GRANTED_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected GRANTED_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export function buildWebSearchProviderConfig(params: {
  provider: NonNullable<
    NonNullable<NonNullable<NonNullable<GrantedConfig["tools"]>["web"]>["search"]>["provider"]
  >;
  enabled?: boolean;
