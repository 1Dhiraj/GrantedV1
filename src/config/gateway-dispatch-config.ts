...
function projectGatewayDispatchConfig(value: unknown): GrantedConfig {
  if (!isRecord(value)) {
    return {};
  }
  ...
  return projected as GrantedConfig;
}

// Main session keys are process-local; Gateway dispatch always sees the canonical main key.
function applyGatewayDispatchSessionDefaults(config: GrantedConfig): GrantedConfig {
...
function resolveGatewayDispatchEnvVars(config: unknown, env: NodeJS.ProcessEnv): unknown {
  if (isRecord(config) && Object.hasOwn(config, "env")) {
    applyConfigEnvVars(config as GrantedConfig, env);
  }
