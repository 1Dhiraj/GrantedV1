...
describe("sessions", () => {
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ GRANTED_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from GRANTED_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDirForAgent(
      "main",
      { GRANTED_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when GRANTED_STATE_DIR differs from stored paths", () => {
