...
let listedConfigs: Array<GrantedConfig | undefined> = [];
...
function runGenerateVideo(params: GenerateVideoParams) {
  const defaults = params.cfg.agents?.defaults as
    | (NonNullable<GrantedConfig["agents"]>["defaults"] & {
...
describe("video-generation runtime", () => {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(
      listRuntimeVideoGenerationProviders({ config: {} as GrantedConfig }, runtimeDeps),
    ).toEqual(registryProviders);
    expect(listedConfigs).toEqual([{} as GrantedConfig]);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    await expect(
      runGenerateVideo({ cfg: {} as GrantedConfig, prompt: "animate a cat" }),
    ).rejects.toThrow(
