...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.GRANTED_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.GRANTED_DISABLE_BONJOUR) &&
  ...
  const sshPort = mdnsMinimal
    ? undefined
    : (parseTcpPort(process.env.GRANTED_SSH_PORT) ?? undefined);
