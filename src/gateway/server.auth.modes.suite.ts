...
export function registerAuthModesSuite(): void {
  ...
  describe("startup auth validation", () => {
    test.each([
      {
        mode: "token" as const,
        envKey: "GRANTED_GATEWAY_TOKEN" as const,
        expected:
          "gateway auth mode is token, but no token was configured (set gateway.auth.token or GRANTED_GATEWAY_TOKEN)",
      },
      {
        mode: "password" as const,
        envKey: "GRANTED_GATEWAY_PASSWORD" as const,
    ...
    test("rejects non-loopback exposure without effective auth before listening", async () => {
      ...
      await expect(
        ...
        "without auth (set gateway.auth.token/password, or set GRANTED_GATEWAY_TOKEN/GRANTED_GATEWAY_PASSWORD",
      );
    });
  });
