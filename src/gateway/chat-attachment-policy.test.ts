...
const cfgWithMediaMaxMb = (value: unknown): GrantedConfig =>
...
describe("resolveChatAttachmentMaxBytes", () => {
  ...
  it("falls back to the default ceiling when unset", () => {
    expect(resolveChatAttachmentMaxBytes({} as GrantedConfig)).toBe(
      DEFAULT_CHAT_ATTACHMENT_MAX_BYTES,
    );
    expect(resolveChatAttachmentMaxBytes({ agents: {} } as unknown as GrantedConfig)).toBe(
      DEFAULT_CHAT_ATTACHMENT_MAX_BYTES,
    );
  });
...
describe("resolveChatAttachmentPolicy", () => {
  ...
  it("clamps the advertised ceiling to what one WS frame can carry as base64", () => {
    ...
    expect(resolveChatAttachmentPolicy({} as GrantedConfig).maxBytes).toBe(MAX_ADVERTISED_BYTES);
