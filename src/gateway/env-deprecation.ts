...
export function warnLegacyOpenClawEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but OpenClaw only reads GRANTED_* names now.`,
      "Rename them by replacing the legacy prefix with GRANTED_; the old names are ignored.",
    ].join("\n"),
    { code: "GRANTED_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}
