...
function getFirecrawlApiKey(config?: GrantedConfig): unknown {
...
function createFirecrawlPluginConfig(apiKey: unknown): GrantedConfig {
  return {
    plugins: {
      entries: {
        firecrawl: {
...
describe("web fetch runtime", () => {
  ...
  it.each(["detached", "published"])(
    ...
    (mode) => {
      ...
      } as GrantedConfig & { tools: { web: { fetch: { provider: string } } } };
