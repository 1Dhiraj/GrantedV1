...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: GrantedConfig, policy: ChannelAccessPolicy) => GrantedConfig;
  resolveAllowlist: (params: { cfg: GrantedConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: GrantedConfig; resolved: TResolved }) => GrantedConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: GrantedConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: GrantedConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: GrantedConfig, policy: ChannelAccessPolicy): GrantedConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: GrantedConfig, policy: ChannelAccessPolicy): GrantedConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: GrantedConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: GrantedConfig; resolved: string[] }) => {
      calls.push("apply");
