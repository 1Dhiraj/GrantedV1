...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: GrantedConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: GrantedConfig }>(
  collector: WarningCollector<{ cfg: GrantedConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: GrantedConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: GrantedConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: GrantedConfig },
>(
  projectCfg: (cfg: GrantedConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: GrantedConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: GrantedConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: GrantedConfig },
>(params: {
  providerConfigPresent: (cfg: GrantedConfig) => boolean;
