...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): GrantedConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as GrantedConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: GrantedConfig) => string[];
  input: GrantedConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: GrantedConfig, expected: string) {
...
describe("createAccountListHelpers", () => {
  ...
  describe("listAccountIds", () => {
    ...
    it("does not synthesize an account when an owner disables the empty fallback", () => {
      ...
      expect(helpers.listAccountIds({} as GrantedConfig)).toEqual([]);
      ...
      expect(helpers.resolveDefaultAccountId({} as GrantedConfig)).toBe("default");
    });
