...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(channelSupportsMessageCapability({} as GrantedConfig, "presentation")).toBe(true);
    expect(channelSupportsMessageCapability({} as GrantedConfig, "delivery-pin")).toBe(true);
  });
  ...
  it("normalizes channel aliases for per-channel capability checks", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as GrantedConfig, channel: "demo-cards-alias" },
        "delivery-pin",
      ),
    ).toBe(true);
  });
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(channelSupportsMessageCapability({} as GrantedConfig, "presentation")).toBe(false);
