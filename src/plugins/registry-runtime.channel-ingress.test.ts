...
describe("bundled channel ingress runtime ownership", () => {
  it("binds authenticated owner turns to the exact live trusted channel plugin", async () => {
    ...
    const ownerApi = registryBuilder.createApi(owner, { config: {} as GrantedConfig });
    const foreignApi = registryBuilder.createApi(foreign, { config: {} as GrantedConfig });
    const untrustedApi = registryBuilder.createApi(untrusted, { config: {} as GrantedConfig });
