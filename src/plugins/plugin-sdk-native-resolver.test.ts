...
function writeFakeOpenClawPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "granted.mjs"), "#!/usr/bin/env node\n", "utf8");
...
describe("installOpenClawPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    ...
    const previousDevSourceRoot = process.env.GRANTED_DEV_SOURCE_ROOT;
    process.env.GRANTED_DEV_SOURCE_ROOT = devRoot;
    ...
    try {
      ...
      if (previousDevSourceRoot === undefined) {
        delete process.env.GRANTED_DEV_SOURCE_ROOT;
      } else {
        process.env.GRANTED_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
