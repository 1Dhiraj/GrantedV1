...
describe("stageBundledPluginRuntime", () => {
  ...
  it("copies package metadata files but symlinks other non-js plugin artifacts into the runtime overlay", () => {
    ...
    setupRepoFiles(repoRoot, {
      [bundledDistPluginFile("diffs", "package.json")]: JSON.stringify(
        { name: "@granted/diffs", openclaw: { extensions: ["./index.js"] } },
        null,
        2,
      ),
      [bundledDistPluginFile("diffs", "granted.plugin.json")]: "{}\n",
    ...
    expectRuntimeArtifactText({
      repoRoot,
      pluginId: "diffs",
      relativePath: "granted.plugin.json",
      expectedText: "{}\n",
      symbolicLink: false,
    });
    expectRuntimeArtifactText({
  ...
  it("preserves package metadata needed for bundled plugin discovery from dist-runtime", () => {
    ...
    setupRepoFiles(repoRoot, {
      ...
      [bundledDistPluginFile("demo", "granted.plugin.json")]: JSON.stringify(
        {
          id: "demo",
          channels: ["demo"],
    ...
    const env = {
      ...
      GRANTED_BUNDLED_PLUGINS_DIR: runtimeExtensionsDir,
    };
