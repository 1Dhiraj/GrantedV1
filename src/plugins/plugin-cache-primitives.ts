...
export type ConfigScopedRuntimeCache<T> = WeakMap<GrantedConfig, Map<string, T>>;
...
type ConfigScopedPromiseLoader<T> = {
  load(config?: GrantedConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: GrantedConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<GrantedConfig, Promise<T>>();

  const createPromise = (config?: GrantedConfig): Promise<T> => {
  ...
  const loader: ConfigScopedPromiseLoader<T> = {
    async load(config?: GrantedConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<GrantedConfig, Promise<T>>();
    },
  };
