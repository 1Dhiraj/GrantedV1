...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: GrantedPluginApi | undefined;
    let workspaceApi: GrantedPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    try {
      ...
      let neverActiveApi: GrantedPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: GrantedPluginApi | undefined;
      registerTestPlugin({
