...
function metadataSnapshot(params: {
  ...
  const packageName =
    params.packageName === null ? undefined : (params.packageName ?? `@granted/${id}`);
...
describe("plugin management Featured authority", () => {
  ...
  it("surfaces a newly featured live official package without static fallback metadata", async () => {
    ...
    expect(catalog.plugins).toEqual([
      expect.objectContaining({
        ...
        name: "New Tool",
        featured: true,
        install: { source: "official", pluginId: "@granted/new-tool" },
      }),
    ]);
  });
  ...
  it("orders live featured packages by when they were featured", async () => {
    ...
    mocks.officialCatalog.mockResolvedValue(
      hostedCatalog([
        hostedFeedEntry({
          packageName: "@granted/older-popular",
          title: "Older Popular",
          featured: true,
          featuredAt: 100,
          order: 1,
        ...
        hostedFeedEntry({
          packageName: "@granted/newest-featured",
        ...
        hostedFeedEntry({
          packageName: "@granted/legacy-featured",
    ...
    expect(catalog.plugins.map((plugin) => plugin.id)).toEqual([
      "@granted/newest-featured",
      "@granted/older-popular",
      "@granted/legacy-featured",
    ]);
  ...
  it("lets a live unfeature override an installed published plugin manifest", async () => {
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        ...
        name: "Diffs",
        origin: "global",
        installRecord: { source: "npm", spec: "@granted/diffs" },
      }),
    );
    mocks.officialCatalog.mockResolvedValue(
      hostedCatalog([{ ...hostedFeedDiffsEntry, featured: false }]),
  ...
  it.each([
    { id: "workboard", name: "Workboard", packageName: "@granted/workboard" },
    { id: "memory-wiki", name: "Memory Wiki", packageName: "@granted/memory-wiki" },
  ...
  it("preserves npm-only bundled curation outside the hosted producer identity", async () => {
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        ...
        installRecord: { source: "npm", spec: "@granted/acpx" },
      }),
    );
    mocks.officialCatalog.mockResolvedValue(hostedCatalog([]));
    ...
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        ...
        installRecord: {
          ...
          clawhubPackage: "@granted/new-tool",
        },
      }),
    );
    mocks.officialCatalog.mockResolvedValue(hostedCatalog([]));
