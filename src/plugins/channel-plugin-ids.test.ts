...
describe("resolveGatewayStartupPluginIdsFromRegistry", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds.mockReset().mockImplementation((config: GrantedConfig) => {
    ...
    listPotentialConfiguredChannelPresenceSignals
      .mockReset()
      .mockImplementation((config: GrantedConfig) => {
  ...
  it.each([
    ...
    [
      ...
      { channels: {}, talk: { providers: { edge: {} } } } as GrantedConfig,
      ["browser", "microsoft", "memory-core"],
    ],
    [
      "activates the selected Talk realtime capability alias",
      { channels: {}, talk: { realtime: { provider: "grok-voice" } } } as GrantedConfig,
  ...
  it("loads bundled worker-provider owners required by durable environments", () => {
    expectStartupPluginIds({
      config: { channels: {} } as GrantedConfig,
  ...
  it("keeps durable worker-provider owners behind disable and allowlist gates", () => {
    expectStartupPluginIds({
      config: { channels: {}, plugins: { enabled: false } } as GrantedConfig,
    ...
    expectStartupPluginIds({
      config: { channels: {}, plugins: { deny: ["qa-lab"] } } as GrantedConfig,
      workerProviderIds: ["static-ssh"],
      expected: ["browser", "memory-core"],
    });
    expectStartupPluginIds({
      config: { channels: {}, plugins: { allow: ["browser"] } } as GrantedConfig,
      workerProviderIds: ["static-ssh"],
      expected: ["browser"],
    });
  });
  ...
  it("keeps startup-lazy external plugins behind config and activation policy", () => {
    ...
    const cases: Array<{ plugins: GrantedConfig["plugins"]; expected: readonly string[] }> = [
      {
        plugins: {
          slots: { memory: "none" },
          entries: {
    ...
    for (const testCase of cases) {
      expectStartupPluginIds({
        config: { channels: {}, plugins: testCase.plugins } as GrantedConfig,
...
describe("listConfiguredChannelIdsForReadOnlyScope", () => {
  ...
  it("does not let namespace discovery bypass an incomplete trusted channel contract", () => {
    ...
    expect(
      resolveConfiguredChannelPresencePolicy({
        config: { plugins: { allow: ["external-env-channel-plugin"] } } as GrantedConfig,
        workspaceDir: "/tmp",
  ...
  it.each(["global", "config"] as const)(
    ...
    (origin) => {
      ...
      const config = { plugins: { allow: ["slack"] } } as GrantedConfig;
