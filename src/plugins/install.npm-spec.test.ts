...
beforeAll(async () => {
  ...
  const dependencySpec = managedManifest.dependencies?.["@granted/pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  ...
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@granted/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@granted/pack-demo@1.2.3");
  ...
  it.each(["commit", "rollback"] as const)(
    ...
    async (settlement) => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      const packageName = "@granted/pack-demo";
  ...
  it("installs npm plugins into .openclaw/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@granted/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@granted/voice-call@0.0.1");
  ...
  it.each(["omitted", "missing package manifest", "missing native executable"])(
    ...
    async (initialPackageState) => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      const packageName = "@granted/codex-fixture";
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@granted/voice-call";
    const warnings: string[] = [];
    const npmProjectRoot = resolvePluginNpmProjectDir({ npmDir: npmRoot, packageName });
    const stalePackageDir = path.join(npmProjectRoot, "node_modules", "stale-plugin");
    fs.mkdirSync(stalePackageDir, { recursive: true });
  ...
  it("rejects managed npm plugins when their openclaw peer link cannot be repaired", async () => {
    ...
    mockNpmViewAndInstall({
      spec: "@granted/codex@2026.5.7",
      packageName: "@granted/codex",
      version: "2026.5.7",
      pluginId: "@granted/codex",
      npmRoot,
    ...
    expect(result.error).toContain("@granted/codex");
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@granted/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@granted/codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("GRANTED_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@granted/whatsapp@2026.5.27",
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@granted/whatsapp@2026.5.26",
        packageName: "@granted/whatsapp",
        version: "2026.5.26",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@granted/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @granted/whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@granted/whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    expect(managedManifest.dependencies?.["@granted/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@granted/msteams@2026.5.28-beta.3",
        packageName: "@granted/msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@granted/msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @granted/msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@granted/msteams",
    ...
    expect(managedManifest.dependencies?.["@granted/msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it("repairs stale managed openclaw root packages before npm plugin installs", async () => {
    ...
    expect(manifest.dependencies?.["@granted/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@granted/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
  ...
  it("does not fail rollback snapshots on plugin-local openclaw peer symlinks", async () => {
    ...
    writeNpmRootPackageLock({
      npmRoot: npmProjectRoot,
      dependencies: { "@granted/codex": "0.0.1" },
      packages: [
        {
          packageName: "@granted/codex",
          version: "0.0.1",
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@granted/codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
              name: "@granted/codex",
              version: "0.0.2",
  ...
  it.each([
    {
      ...
      packageName: "@granted/comfy-provider",
      expectedPluginId: "comfy",
      nativeManifest: "missing",
    },
    {
      name: "missing manifest even when the package name matches the expected id",
      packageName: "@granted/official-placeholder",
      expectedPluginId: "@granted/official-placeholder",
      nativeManifest: "missing",
    },
    {
      name: "malformed manifest",
      packageName: "@granted/official-malformed",
  ...
  it("accepts a trusted manifest-declared plugin id replacement during update", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    mockNpmViewAndInstall({
      spec: "@granted/fish-audio-speech@2026.8.1-beta.0",
      packageName: "@granted/fish-audio-speech",
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@granted/voice-call");
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@granted/voice-call"))).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@granted/whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@granted/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@granted/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @granted/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@granted/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@granted/voice-call@0.0.2-beta.1",
        packageName: "@granted/voice-call",
        version: "0.0.2-beta.1",
        pluginId: "voice-call",
        npmRoot: prereleaseOnlyNpmRoot,
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@granted/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @granted/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@granted/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@granted/voice-call",
