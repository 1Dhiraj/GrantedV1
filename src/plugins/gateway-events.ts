...
export type GrantedPluginGatewayEventScope = "operator.read" | "operator.write" | "operator.admin";

export type GrantedPluginSessionsChangedEvent = {
  sessionKey: string;
  agentId?: string;
  label?: string;
  displayName?: string;
...
type SessionsChangedHandler = (event: GrantedPluginSessionsChangedEvent) => unknown;
...
export type GrantedPluginGatewayEvents = {
  emit: (
    event: string,
    payload: PluginJsonValue,
    opts: { scope: GrantedPluginGatewayEventScope },
  ) => void;
  /**
   ...
   */
  onSessionsChanged: (handler: (event: GrantedPluginSessionsChangedEvent) => void) => () => void;
};

export function subscribePluginSessionsChanged(
  handler: (event: GrantedPluginSessionsChangedEvent) => void,
): () => void {
...
export function queuePluginSessionsChanged(payload: unknown): void {
  ...
  const event: GrantedPluginSessionsChangedEvent = {
