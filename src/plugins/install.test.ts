...
async function ensureDynamicArchiveTemplate(params: {
  ...
  if (params.writePluginManifest !== false) {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "granted.plugin.json"),
      JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "granted.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  ...
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@granted/voice-call" });
  ...
  it("rejects native plugin zip archives without granted.plugin.json", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    ...
    expect(fs.existsSync(resolvePluginInstallDir("@granted/zipper", extensionsDir))).toBe(false);
  });
  ...
  it("rejects packages without openclaw.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@granted/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
  ...
  it("rejects legacy plugin package shape when openclaw.extensions is missing", async () => {
    ...
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@granted/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
...
describe("installPluginFromDir", () => {
  ...
  it("emits a redacted security event after installing a plugin directory", async () => {
    ...
    expect(captured.events[0]).toMatchObject({
      ...
      severity: "medium",
      actor: { kind: "operator" },
      target: { kind: "plugin", name: "@granted/test-plugin" },
  ...
  it("allows plugins when a beta host is on the package plugin API floor", async () => {
    ...
    expect(result.pluginId).toBe("@granted/test-plugin");
  });

  it("uses granted.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@granted/cognee-openclaw"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@granted/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
