...
const ALLOWED_PLUGIN_SDK_FIXTURE_IMPORTS = new Set([
  ...
  'src/plugins/loader.git-path-regression.test.ts:`import { resolveOutboundSendDep } from "granted/plugin-sdk/channel-outbound";',
  ...
  'src/plugins/loader.test.ts:`import { normalizeLowercaseStringOrEmpty } from "granted/plugin-sdk/string-coerce-runtime";`,',
]);
...
function findLoaderFixtureSdkImports(): string[] {
  ...
  for (const file of LOADER_FIXTURE_TEST_FILES) {
    ...
    for (const line of source.split("\n")) {
      if (
        line.includes('require("granted/plugin-sdk') ||
