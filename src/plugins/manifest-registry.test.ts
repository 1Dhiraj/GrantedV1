...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "granted.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createMsteamsClawHubInstallRecord(
  ...
  const record: PluginInstallRecord = {
    source: "clawhub",
    spec: "clawhub:@granted/msteams",
    installPath,
    clawhubUrl: "https://clawhub.ai",
    clawhubPackage: "@granted/msteams",
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "granted.plugin.json");
  const linkedManifest = path.join(rootDir, "granted.plugin.json");
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("preserves the identity of every independently malformed plugin manifest", () => {
    const candidates = ["first-invalid", "second-invalid"].map((pluginId) => {
      const rootDir = makeTempDir();
      fs.writeFileSync(path.join(rootDir, "granted.plugin.json"), '{"id":', "utf-8");
    ...
    expect(registry.diagnostics).toEqual(
      candidates.map((candidate) =>
        expect.objectContaining({
          level: "error",
          pluginId: "index",
          source: path.join(candidate.rootDir, "granted.plugin.json"),
        }),
      ),
    );
  });
  ...
  it.each([
    { name: "complete records", overrides: {} },
    {
      ...
      overrides: { spec: "clawhub:@granted/msteams@2026.6.11" },
    },
    {
    ...
    },
    {
      ...
      overrides: { resolvedSpec: "@granted/msteams@2026.6.11" },
    },
    {
      name: "matching ClawHub resolved specs",
      overrides: { resolvedSpec: "clawhub:@granted/msteams@2026.6.11" },
    },
    {
      name: "matching resolved package names",
      overrides: { resolvedName: "@granted/msteams" },
    },
  ] satisfies Array<{ name: string; overrides: Partial<PluginInstallRecord> }>)(
  ...
  it.each([
    ...
    },
    {
      ...
      overrides: { clawhubPackage: "@granted/line" },
    },
    {
      name: "conflicting requested spec",
      overrides: { spec: "clawhub:@granted/line" },
    },
    {
      name: "conflicting npm resolved spec",
      overrides: { resolvedSpec: "@granted/line@2026.6.11" },
    },
    {
      name: "conflicting ClawHub resolved spec",
      overrides: { resolvedSpec: "clawhub:@granted/line@2026.6.11" },
    },
    {
    ...
    },
    {
      ...
      overrides: { clawhubPackage: "@granted/msteams@2026.6.11" },
    },
    {
      name: "malformed requested spec",
      overrides: { spec: "@granted/msteams" },
    },
    {
    ...
    },
    {
      ...
      overrides: { resolvedName: "@granted/line" },
    },
    {
      name: "malformed resolved package name",
      overrides: { resolvedName: "@granted/msteams@2026.6.11" },
    },
    {
    ...
    },
    {
      ...
      overrides: {
        clawhubPackage: undefined,
        spec: undefined,
        resolvedSpec: "@granted/msteams@2026.6.11",
      },
    },
  ] satisfies Array<{ name: string; overrides: Partial<PluginInstallRecord> }>)(
  ...
  it("does not trust legacy ClawHub records without source authority", () => {
    ...
    const registry = loadPluginManifestRegistryCore({
      installRecords: {
        "diagnostics-otel": {
          source: "clawhub",
          spec: "clawhub:@granted/diagnostics-otel@2026.5.18",
          installPath: dir,
        },
      },
      candidates: [
    ...
    const registry = loadPluginManifestRegistryCore({
      installRecords: {
        "diagnostics-otel": {
          ...
          installPath: dir,
          resolvedName: "@granted/diagnostics-otel",
          resolvedVersion: "2026.5.18",
          resolvedSpec: "@granted/diagnostics-otel@2026.5.18",
        },
      },
      candidates: [
        createPluginCandidate({
    ...
    const registry = loadPluginManifestRegistryCore({
      installRecords: {
        "diagnostics-prometheus": {
          source: "npm",
          installPath: dir,
          resolvedName: "@granted/diagnostics-prometheus",
  ...
  it("reports non-bundled channel manifests without channel config descriptors", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(dir, "granted.plugin.json"),
  ...
  it("ignores provider catalog entries outside the plugin root", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(pluginDir, "granted.plugin.json"),
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "granted.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
