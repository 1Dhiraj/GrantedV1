...
import { normalizeOptionalString as normalizeString } from "@granted/normalization-core/string-coerce";
...
function getConfigPath(config: GrantedConfig | undefined, path: string): unknown {
...
function setConfigPath(target: GrantedConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: GrantedConfig) => getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: GrantedConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: GrantedConfig) =>
