...
describe("loadPluginRegistrySnapshotWithMetadata", () => {
  it.each(["persisted", "current"])(
    ...
    (cache) => {
      ...
      writePersistedInstalledPluginIndexSync(index, { stateDir: env.GRANTED_STATE_DIR });
      if (cache === "current") {
      ...
      const result = loadPluginRegistrySnapshotWithMetadata({
        ...
        env: { ...env, GRANTED_DEV_SOURCE_ROOT: packageRoot },
      });
  ...
  it("keeps persisted manifestless Claude bundles on the fast path", () => {
    ...
    const stateDir = path.join(tempRoot, "state");
    const env = { ...createHermeticEnv(tempRoot), GRANTED_DISABLE_BUNDLED_PLUGINS: "1" };
    const config = {
      plugins: {
        load: { paths: [rootDir] },
      },
