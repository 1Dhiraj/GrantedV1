...
vi.mock("./plugin-metadata-snapshot.js", () => ({
  loadPluginMetadataSnapshot: (params: { config?: GrantedConfig; env?: NodeJS.ProcessEnv }) => {
...
describe("migration provider runtime", () => {
  ...
  it("standalone-loads bundled migration providers through compat config", () => {
    ...
    ensureStandaloneMigrationProviderRegistryLoaded({
      cfg: { plugins: { enabled: false } } as GrantedConfig,
    });
  ...
  it.each(["owner", "retired"])(
    ...
    async (change) => {
      ...
      const cfgA = { plugins: { allow: ["migration-a"] } } as GrantedConfig;
      const cfgB = { plugins: { allow: ["migration-b"] } } as GrantedConfig;
