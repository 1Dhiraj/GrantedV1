...
function mergePackageChannelMetaIntoChannelConfigs(params: {
  ...
  packageChannel?: GrantedPackageManifest["channel"];
...
function pushNonBundledChannelConfigDescriptorDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message: `channel plugin manifest declares ${safeMissingChannels.join(", ")} without channelConfigs metadata; add granted.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads. Channels without channelConfigs still appear in channel listings, but setup UI may be limited.`,
  });
}
    ...
    if (candidate.origin !== "bundled") {
      ...
      if (
        ...
        diagnostics.push({
          ...
          pluginId: effectivePluginId,
          source: packageManifestSource,
          message: `plugin requires plugin API ${packagePluginApiRange}, but this host is ${currentHostVersion}; skipping load (check "openclaw --version", GRANTED_COMPATIBILITY_HOST_VERSION, or run "openclaw doctor")`,
        });
        continue;
      }
    }
