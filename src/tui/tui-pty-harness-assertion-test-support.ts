...
async function exerciseSelectorOutputSafety(
  ...
  const fixture = await startFixture({
    env: {
      GRANTED_TUI_PTY_COLS: "240",
      GRANTED_TUI_PTY_ROWS: "24",
      GRANTED_TUI_PTY_MODEL: "fixture-provider/fixture-model",
      GRANTED_TUI_PTY_PICKER_FIXTURE: "1",
      GRANTED_TUI_PTY_PICKER_MODEL_VALUE: selectedModel,
      GRANTED_TUI_PTY_PICKER_MODEL_NAME: modelName.text,
      GRANTED_TUI_PTY_PICKER_SESSION_KEY: selectedSessionKey,
      GRANTED_TUI_PTY_PICKER_SESSION_TITLE: sessionTitle.text,
      GRANTED_TUI_PTY_PICKER_SESSION_PREVIEW: sessionPreview.text,
      GRANTED_TUI_PTY_PICKER_SESSION_DISPLAY_NAME: sessionDisplay.text,
    },
  });
...
export async function exerciseNarrowTerminalRendering(
  ...
  const fixture = await startFixture({
    env: {
      GRANTED_TUI_PTY_COLS: "28",
      GRANTED_TUI_PTY_ROWS: "18",
      GRANTED_TUI_PTY_INITIAL_MESSAGE: message,
    },
  });
...
async function exerciseGatewayOutputSafety(
  ...
  const fixture = await startFixture({
    env: {
      GRANTED_TUI_PTY_COLS: "120",
      GRANTED_TUI_PTY_ROWS: "18",
      GRANTED_TUI_PTY_GATEWAY_STATUS: systemAttacks.join(""),
      GRANTED_TUI_PTY_DISCONNECT_REASON: idlePayload.text,
    },
  });
...
async function exerciseMarkdownAndAutocompleteOutputSafety(
  ...
  const fixture = await startFixture({
    env: {
      GRANTED_TUI_PTY_COLS: "140",
      GRANTED_TUI_PTY_DYNAMIC_COMMAND_DESCRIPTION: command.text,
      GRANTED_TUI_PTY_IN_FLIGHT_TEXT: `**${inFlight.text}** [copy-safe](https://example.test/t08-inflight)`,
      GRANTED_TUI_PTY_ROWS: "22",
      GRANTED_TUI_PTY_SAFE_THINKING_LABEL: "T08_SAFE_THINKING",
      GRANTED_TUI_PTY_THINKING_LABEL: thinking.text,
    },
  });
...
async function exerciseInteractiveOutputSafety(
  ...
  const fixture = await startFixture({
    env: {
      GRANTED_TUI_PTY_BTW_QUESTION: btwPayload.text,
      GRANTED_TUI_PTY_COLS: "120",
      GRANTED_TUI_PTY_MODEL: "fixture-provider/fixture-model",
      GRANTED_TUI_PTY_ROWS: "20",
      GRANTED_TUI_PTY_TOOL_NAME: toolPayload.text,
      GRANTED_TUI_PTY_VERBOSE_LEVEL: "on",
    },
  });
...
export async function exerciseFragmentedUnicodePrompt(
  ...
  const fixture = await startFixture({
    env: { GRANTED_TUI_PTY_TYPE_CHUNK_SIZE: "1", GRANTED_TUI_PTY_TYPE_DELAY_MS: "1" },
  });
