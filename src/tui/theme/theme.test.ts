...
type ThemeEnvOverrides = {
  GRANTED_THEME?: string | undefined;
...
async function importThemeWithEnv(env: ThemeEnvOverrides) {
  if (Object.hasOwn(env, "GRANTED_THEME")) {
    vi.stubEnv("GRANTED_THEME", env.GRANTED_THEME);
  }
...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      GRANTED_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when GRANTED_THEME=light", async () => {
    const mod = await importThemeWithEnv({ GRANTED_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when GRANTED_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ GRANTED_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats GRANTED_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ GRANTED_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("GRANTED_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      GRANTED_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ GRANTED_THEME: "light" });
    const darkMod = await importThemeWithEnv({ GRANTED_THEME: "dark" });
