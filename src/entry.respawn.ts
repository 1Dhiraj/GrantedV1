...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.GRANTED_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[GRANTED_NODE_EXTRA_CA_CERTS_READY]) &&
    !childEnv.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[GRANTED_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !shouldSkipRespawnForArgv(argv, platform) &&
    !isTruthyEnvValue(env[GRANTED_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[GRANTED_NODE_OPTIONS_READY] = "1";
