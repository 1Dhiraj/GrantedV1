...
export type SpeechProviderRegistryResolver = {
  getProvider: (providerId: string, cfg?: GrantedConfig) => SpeechProviderPlugin | undefined;
  listProviders: (cfg?: GrantedConfig) => SpeechProviderPlugin[];
};
...
export function createSpeechProviderRegistry(resolver: SpeechProviderRegistryResolver) {
  const buildAliasIndex = (cfg?: GrantedConfig) =>
    buildCapabilityProviderIndex(resolver.listProviders(cfg), "aliases");

  const listProviders = (cfg?: GrantedConfig): SpeechProviderPlugin[] => [
