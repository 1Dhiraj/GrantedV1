...
import {
  ...
  type GrantedPluginApiWithoutFacades,
} from "../plugins/api-facades.js";
import type { GrantedPluginApi } from "./plugin-runtime.js";

/** Partial plugin API overrides accepted by the SDK test helper. */
export type TestPluginApiInput = Partial<GrantedPluginApi>;

/** Create a minimal plugin API object for plugin-sdk contract and unit tests. */
export function createTestPluginApi(api: TestPluginApiInput = {}): GrantedPluginApi {
  ...
  const mergedApi = {
    ...
    on() {},
    ...flatApi,
  } satisfies GrantedPluginApiWithoutFacades;
