...
import type { GrantedPluginApi, WorkerMachineOption } from "./plugin-entry.js";
import type { PluginHookAgentTrigger } from "./types.js";

function registerScopedReplyHook(api: GrantedPluginApi): void {
...
function registerAuthorizedPromptHook(api: GrantedPluginApi): void {
  api.on(
    "before_prompt_build",
    async (_event, ctx) => {
...
describe("plugin entry hook option contracts", () => {
  it("exposes scoped reply and prompt authority options through the public plugin API", () => {
    expectTypeOf<GrantedPluginApi["on"]>().toBeFunction();
