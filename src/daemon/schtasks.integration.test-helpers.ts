...
async function inspectNativeAclDenial(params: {
  ...
  const scriptEnv = { ...process.env, GRANTED_TASK_SCRIPT: params.scriptPath };
  ...
  const dotnetOpenRead = inspectNativeAclCommand(
    ...
    [
      ...
      "-NonInteractive",
      "-Command",
      '$ErrorActionPreference="Stop"; $stream=[System.IO.File]::OpenRead($env:GRANTED_TASK_SCRIPT); try { [void]$stream.ReadByte() } finally { $stream.Dispose() }',
    ],
    scriptEnv,
  );
  const cmdType = inspectNativeAclCommand(
    getWindowsCmdExePath(),
    ["/d", "/s", "/v:off", "/c", '"type "%GRANTED_TASK_SCRIPT%""'],
    scriptEnv,
    true,
  );
  ...
  const cmdBatchResult = inspectNativeAclCommand(
    getWindowsCmdExePath(),
    ["/d", "/s", "/v:off", "/c", '""%GRANTED_TASK_SCRIPT%""'],
    scriptEnv,
    true,
  );
...
export async function proveNativeStartupFallbackLaunch(params: {
  ...
  const stateDir = path.join(proofRoot, "state & %GRANTED_STARTUP_PROBE% !");
  ...
  await fs.writeFile(
    ...
    [
      ...
      `const env = ${JSON.stringify({
        APPDATA: env.APPDATA,
        GRANTED_CONFIG_PATH: env.GRANTED_CONFIG_PATH,
        GRANTED_PROFILE: env.GRANTED_PROFILE,
