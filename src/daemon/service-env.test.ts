...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.GRANTED_GATEWAY_PORT).toBe("18789");
    expect(env.GRANTED_GATEWAY_TOKEN).toBeUndefined();
    expect(env.GRANTED_SERVICE_MARKER).toBe("granted");
    expect(env.GRANTED_SERVICE_KIND).toBe("gateway");
    expect(env).not.toHaveProperty("GRANTED_SERVICE_VERSION");
    expect(env.GRANTED_SYSTEMD_UNIT).toBe("granted-gateway.service");
    expect(env.GRANTED_WINDOWS_TASK_NAME).toBe("Granted Gateway");
    expect(env.GRANTED_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    if (process.platform === "darwin") {
      expect(env.GRANTED_LAUNCHD_LABEL).toBe("ai.granted.gateway");
    }
  });
  ...
  it("passes through GRANTED_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.GRANTED_WRAPPER).toBe("/usr/local/bin/openclaw-doppler");
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", GRANTED_PROFILE: "work" },
      port: 18789,
    });
    expect(env.GRANTED_SYSTEMD_UNIT).toBe("granted-gateway-work.service");
    expect(env.GRANTED_WINDOWS_TASK_NAME).toBe("Granted Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.GRANTED_LAUNCHD_LABEL).toBe("ai.granted.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        GRANTED_SYSTEMD_UNIT: "granted-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.GRANTED_SYSTEMD_UNIT).toBe("granted-gateway-maintenance.service");
  });
  ...
  it("preserves explicit systemd unit overrides with service suffix", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        GRANTED_SYSTEMD_UNIT: "granted-gateway-maintenance.service",
      },
      port: 18789,
      platform: "linux",
    });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", GRANTED_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    ...
    expect(env.GRANTED_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
  ...
  it.each([
    ...
    ({ capacityMiB, wrapper, expected }) => {
      ...
      try {
        expect(
          buildServiceEnvironment({
            env: { HOME: "/home/user", GRANTED_WRAPPER: wrapper },
            port: 18789,
            runtime: "bun",
          }).NODE_OPTIONS,
        ).toBe(expected);
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the OpenClaw-owned launchd marker for macOS node services", () => {
    ...
    expect(env.GRANTED_LAUNCHD_LABEL).toBe("ai.granted.node");
  });
  ...
  it("fences inherited Node compile cache in macOS node LaunchAgents", () => {
    ...
    const plist = buildLaunchAgentPlist({
      ...
      stdoutPath: "/tmp/granted-node.log",
      stderrPath: "/tmp/granted-node.err.log",
      environment,
    });
  ...
  it("passes through GRANTED_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", GRANTED_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.GRANTED_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through GRANTED_GATEWAY_PASSWORD for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", GRANTED_GATEWAY_PASSWORD: " node-password " },
    });
    expect(env.GRANTED_GATEWAY_PASSWORD).toBe("node-password");
  });
  ...
  it("passes through GRANTED_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", GRANTED_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.GRANTED_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits GRANTED_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.GRANTED_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.GRANTED_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  ...
  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", GRANTED_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", GRANTED_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw"));
  });

  it("uses GRANTED_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", GRANTED_STATE_DIR: "/var/lib/openclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openclaw"));
  });

  it("expands ~ in GRANTED_STATE_DIR", () => {
    const env = { HOME: "/Users/test", GRANTED_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openclaw-state"));
  });

  it("does not interpret $ patterns in HOME when expanding ~ in GRANTED_STATE_DIR", () => {
    const env = { HOME: "/home/$&user", GRANTED_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/home/$&user/openclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { GRANTED_STATE_DIR: "C:\\State\\openclaw" };
