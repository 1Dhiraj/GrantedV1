...
function isLaunchAgentServiceEnvironment(params: {
  ...
  return (
    params.platform === "darwin" && Boolean(params.serviceEnvironment.GRANTED_LAUNCHD_LABEL?.trim())
  );
}
...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  if (params.plan.environment.GRANTED_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.GRANTED_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    GRANTED_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });
