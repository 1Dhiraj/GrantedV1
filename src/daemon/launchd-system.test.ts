...
`,
  ...
  writeFileSync(
    ...
    `#!/bin/sh
printf 'query\\n' >> "$GRANTED_TEST_PROBE_LOG"
query_count=$(/usr/bin/grep -c '^query$' "$GRANTED_TEST_PROBE_LOG")
${launchctlBody}
`,
    { mode: 0o700 },
  );
  ...
  const script = renderSystemLaunchDaemonOwnershipShellProbe("ai.granted.gateway")
  ...
  const output = execFileSync(shell, ["-c", script], {
    ...
    env: {
      ...
      TMPDIR: root,
      PATH: binDir,
      GRANTED_TEST_PROBE_LOG: probeLog,
      GRANTED_TEST_PROBE_DIR: root,
    },
  });
  return {
...
describe("system LaunchDaemon ownership", () => {
  ...
  it("uses the readable system-domain query and reports a loaded owner", async () => {
    ...
    await expect(inspectSystemLaunchDaemonOwnership("ai.granted.gateway")).resolves.toEqual({
      status: "loaded",
      serviceTarget: "system/ai.granted.gateway",
    });
    expect(execLaunchctl).toHaveBeenCalledWith(["print", "system/ai.granted.gateway"], undefined);
  });
  ...
  it("detects the canonical unloaded plist by its structural Label", async () => {
    const plistPath = "/Library/LaunchDaemons/ai.granted.gateway.plist";
    state.files.set(plistPath, "bplist00-binary-payload");
    state.plutilValues.set(plistPath, { Label: "ai.granted.gateway" });
  ...
  it("detects a noncanonical XML plist by its decoded Label", async () => {
    ...
    state.files.set(
      plistPath,
      "<plist><dict><key>Label</key><string>ai.granted.gateway</string></dict></plist>",
    );
    state.plutilValues.set(plistPath, { Label: "ai.granted.gateway" });

    const ownership = await inspectSystemLaunchDaemonOwnership("ai.granted.gateway");
  ...
  it("uses the native top-level Label instead of an earlier nested XML key", async () => {
    ...
    state.files.set(
      plistPath,
      "<plist><dict><key>EnvironmentVariables</key><dict><key>Label</key><string>nested</string></dict><key>Label</key><string>ai.granted.gateway</string></dict></plist>",
    );
    state.plutilValues.set(plistPath, { Label: "ai.granted.gateway" });

    await expect(inspectSystemLaunchDaemonOwnership("ai.granted.gateway")).resolves.toMatchObject({
      status: "installed",
      plistPath,
    });
  });
  ...
  it("skips a valid plist without a string Label and detects a later owner", async () => {
    ...
    state.plutilValues.set(owner, { Label: "ai.granted.gateway" });
  ...
  it("skips an unreadable foreign plist", async () => {
    ...
    await expect(
      assertNoSystemLaunchDaemonOwnership("ai.granted.gateway"),
  ...
  it("can skip the installed-plist scan for read-only status probes", async () => {
    ...
    await expect(
      inspectSystemLaunchDaemonOwnership("ai.granted.gateway", {
  ...
  it("throws one typed recovery error and documents that force cannot bypass it", async () => {
    ...
    const error = await assertNoSystemLaunchDaemonOwnership("ai.granted.gateway").catch(
    ...
    expect(error).toMatchObject({
      ...
      ownership: { status: "loaded", serviceTarget: "system/ai.granted.gateway" },
    });
    ...
    expect(String(error)).toContain("sudo launchctl bootout system/ai.granted.gateway");
  });
  ...
  it.each([
    ...
    [
      "execution failure",
      'printf "Could not find service\\n" >&2\nexec "$GRANTED_TEST_PROBE_DIR/non-executable"',
      1,
      "could not verify",
    ],
    [
      "missing command",
      'printf "Could not find service\\n" >&2\nexec "$GRANTED_TEST_PROBE_DIR/missing"',
      1,
      "could not verify",
    ],
    [
    ...
    (_, body, queries, detail) => {
      ...
      expect(result.conflict).toBe(detail ? "system/ai.granted.gateway" : "");
      if (detail) {
