...
export function isAcpEnabledByPolicy(cfg: GrantedConfig): boolean {
  return cfg.acp?.enabled !== false;
}

/** Resolves the effective dispatch policy state for inbound ACP routing. */
function resolveAcpDispatchPolicyState(cfg: GrantedConfig): AcpDispatchPolicyState {
...
export function resolveAcpDispatchPolicyMessage(cfg: GrantedConfig): string | null {
...
export function resolveAcpDispatchPolicyError(cfg: GrantedConfig): AcpRuntimeError | null {
...
export function resolveAcpExplicitTurnPolicyError(cfg: GrantedConfig): AcpRuntimeError | null {
  if (isAcpEnabledByPolicy(cfg)) {
    return null;
  }
...
function isAcpAgentAllowedByPolicy(cfg: GrantedConfig, agentId: string): boolean {
