...
const EXPECTED_DEFAULT_TAGLINE = "Your AI. Your keys. Your machine.";
...
describe("pickTagline", () => {
  ...
  it("keeps GRANTED_TAGLINE_INDEX behavior in random mode", () => {
    const value = pickTagline({
      mode: "random",
      env: { GRANTED_TAGLINE_INDEX: "0" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(
  ...
  it("ignores partial GRANTED_TAGLINE_INDEX values", () => {
    expect(
      pickTagline({
        mode: "random",
        env: { GRANTED_TAGLINE_INDEX: "1abc" } as NodeJS.ProcessEnv,
        random: () => 0,
      }),
