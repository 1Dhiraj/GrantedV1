...
describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@granted/present"}\n');
      ...
      await expect(
        updateCommandPluginsTesting.collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@granted/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@granted/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@granted/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@granted/missing-install-path@beta",
            },
            local: {
              source: "path",
