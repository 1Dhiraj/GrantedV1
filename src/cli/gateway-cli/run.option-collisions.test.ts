...
describe("gateway run option collisions", () => {
  ...
  it("loads configured shell env fallback before final proxy refresh and gateway startup", async () => {
    await withEnvAsync({ GRANTED_GATEWAY_TOKEN: undefined }, async () => {
      const finalConfig = {
        env: {
          shellEnv: { enabled: true, timeoutMs: 1234 },
          vars: { GRANTED_GATEWAY_TOKEN: "config-token" },
        },
        gateway: {
      ...
      readConfigFileSnapshotWithPluginMetadata
        ...
        .mockImplementationOnce(async (options) => {
          expect(options?.lowerPrecedenceEnv).toEqual({
            GRANTED_GATEWAY_TOKEN: "shell-token",
          });
          expect(process.env.GRANTED_GATEWAY_TOKEN).toBe("shell-token");
          return {
            snapshot: {
              ...configState.snapshot,
              config: {
      ...
      loadShellEnvFallback.mockImplementationOnce((opts?: unknown) => {
        callOrder.push("shell-env");
        (opts as { env: NodeJS.ProcessEnv }).env.GRANTED_GATEWAY_TOKEN = "shell-token";
      });
      const uninstall = installGatewayRunRuntimeHooks({ refreshManagedProxy });
      try {
        await runGatewayCli(["gateway"]);
      ...
      expect(loadShellEnvFallback).toHaveBeenCalledWith({
        enabled: true,
        env: process.env,
        expectedKeys: ["GRANTED_GATEWAY_TOKEN"],
        logger: expect.any(Object),
        timeoutMs: 1234,
      });
      expect(resolveShellEnvExpectedKeys).toHaveBeenCalledWith(
        expect.objectContaining({ GRANTED_GATEWAY_TOKEN: "config-token" }),
        finalConfig,
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledWith(
        expect.objectContaining({
          lowerPrecedenceEnv: { GRANTED_GATEWAY_TOKEN: "shell-token" },
        }),
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledTimes(2);
      expect(process.env.GRANTED_GATEWAY_TOKEN).toBe("config-token");
      expect(clearShellEnvAppliedKeys).toHaveBeenCalledWith(["GRANTED_GATEWAY_TOKEN"]);
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              GRANTED_LOAD_SHELL_ENV: "1",
              GRANTED_SHELL_ENV_TIMEOUT_MS: "4321",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        };
        ...
        shouldEnableShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.GRANTED_LOAD_SHELL_ENV === "1",
        );
        resolveShellEnvFallbackTimeoutMs.mockImplementationOnce((env?: NodeJS.ProcessEnv) =>
          Number(env?.GRANTED_SHELL_ENV_TIMEOUT_MS),
        );
        ...
        expect(process.env.GRANTED_LOAD_SHELL_ENV).toBe("1");
        expect(process.env.GRANTED_SHELL_ENV_TIMEOUT_MS).toBe("4321");
      },
    );
  });
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              GRANTED_DEFER_SHELL_ENV_FALLBACK: "1",
              GRANTED_LOAD_SHELL_ENV: "1",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        ...
        shouldDeferShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.GRANTED_DEFER_SHELL_ENV_FALLBACK === "1",
        );
  ...
  it("ignores shell fallback controls from invalid config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        const invalidConfig = {
          env: { vars: { GRANTED_DEFER_SHELL_ENV_FALLBACK: "1" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
  ...
  it("admits deterministic legacy repairs to gateway preflight and rejects unrelated drift", async () => {
    ...
    await withEnvAsync({ GRANTED_STATE_DIR: undefined }, async () => {
      const stableConfig = {
        meta: {
  ...
  it("replaces config-derived env when the final startup snapshot changes in place", async () => {
    await withEnvAsync(
      {
        GRANTED_GATEWAY_TOKEN: undefined,
        GRANTED_PROXY_URL: undefined,
        GRANTED_RAW_STREAM: undefined,
      },
      async () => {
        const oldConfig = {
          env: {
            vars: {
              ...
              GRANTED_PROXY_URL: "http://127.0.0.1:19876",
              GRANTED_RAW_STREAM: "1",
            },
          },
          gateway: { mode: "local" },
        };
        const newConfig = {
          env: { vars: { GRANTED_GATEWAY_TOKEN: "new-token" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: oldConfig,
        ...
        expect(process.env.GRANTED_GATEWAY_TOKEN).toBe("old-token");
        expect(process.env.GRANTED_PROXY_URL).toBe("http://127.0.0.1:19876");
        ...
        readConfigFileSnapshotWithPluginMetadata.mockImplementationOnce(async () => {
          expect(process.env.GRANTED_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.GRANTED_PROXY_URL).toBeUndefined();
          return { snapshot: configState.snapshot };
        });
        await runGatewayCli(["gateway", "--raw-stream"]);

        expect(process.env.GRANTED_GATEWAY_TOKEN).toBe("new-token");
        expect(process.env.GRANTED_PROXY_URL).toBeUndefined();
        expect(process.env.GRANTED_RAW_STREAM).toBe("1");
      },
    );
  });
  ...
  it("marks descendants when the final config supplies the service marker", async () => {
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: { vars: { GRANTED_SERVICE_MARKER: "openclaw" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: finalConfig,
        ...
        expect(process.env.GRANTED_SERVICE_MARKER).toBe("openclaw");
        expect(process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV]).toBe(String(process.pid));
      },
    );
  });
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    process.env.GRANTED_SERVICE_MARKER = "gateway";
    try {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        ensureDevGatewayConfig.mockImplementationOnce(async () => {
          ...
          expect(process.env.GRANTED_PROFILE).toBe("dev");
          expect(process.env.GRANTED_STATE_DIR).toBeUndefined();
          expect(process.env.GRANTED_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.GRANTED_WORKSPACE_DIR).toBe("/tmp/openclaw-reset-workspace");
          configState.snapshot = {
            exists: true,
            valid: true,
            config: { gateway: { mode: "local" } },
        ...
        loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
          process.env.GRANTED_GATEWAY_TOKEN ??= "trusted-token";
          process.env.GRANTED_PROFILE ??= "dev";
          if (process.env.GRANTED_WORKSPACE_DIR === undefined) {
            setTestEnvValue("GRANTED_WORKSPACE_DIR", "/tmp/openclaw-reset-workspace");
          }
        });
        ...
        expect(process.env.GRANTED_GATEWAY_TOKEN).toBe("trusted-token");
  ...
  it("refuses dev reset if trusted dotenv retargets after pre-bootstrap", async () => {
    await withEnvAsync({ GRANTED_STATE_DIR: "/tmp/openclaw-reset-original" }, async () => {
      configState.snapshot = {
        config: { gateway: { mode: "local" } },
        exists: true,
      ...
      await prepareGatewayReset();
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        setTestEnvValue("GRANTED_STATE_DIR", "/tmp/openclaw-reset-retargeted");
        return {
          dotenvPresentKeys: ["GRANTED_STATE_DIR"],
          gatewayEnvAppliedKeys: [],
          stateEnvAppliedKeys: ["GRANTED_STATE_DIR"],
        };
      });
      ...
      expect(ensureDevGatewayConfig).not.toHaveBeenCalled();
      expect(process.env.GRANTED_STATE_DIR).toBe("/tmp/openclaw-reset-original");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it("blocks a final startup snapshot that changes guarded config selection", async () => {
    await withEnvAsync({ GRANTED_STATE_DIR: undefined }, async () => {
      configState.snapshot = {
        ...
        config: { gateway: { mode: "local" } },
        sourceConfig: {
          env: { vars: { GRANTED_STATE_DIR: "/tmp/openclaw-late-selection" } },
          gateway: { mode: "local" },
        },
      };
  ...
  it("blocks a final startup snapshot that changes an already-selected config selector", async () => {
    await withEnvAsync({ GRANTED_STATE_DIR: undefined }, async () => {
      const guardedConfig = {
        env: { vars: { GRANTED_STATE_DIR: "/tmp/openclaw-guarded-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
      ...
      await selectGatewayRunEnvironment({ opts: {}, runtime: defaultRuntime });
      await prepareGatewayRunBootstrap({ opts: {}, runtime: defaultRuntime });
      expect(process.env.GRANTED_STATE_DIR).toBe("/tmp/openclaw-guarded-state");

      const finalConfig = {
        env: { vars: { GRANTED_STATE_DIR: "/tmp/openclaw-final-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
        config: finalConfig,
  ...
  it.each([
    ...
    delete process.env.GRANTED_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.GRANTED_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
      ...
      withEnvAsync(
        {
          ...
          GRANTED_SERVICE_MANAGED_ENV_KEYS: "OPENAI_API_KEY,ANTHROPIC_API_KEY",
        },
        async () => {
          const { prepareGatewayRunBootstrap, selectGatewayRunEnvironment } =
            await import("./pre-bootstrap.js");
  ...
  it("clears only missing managed keys after reading the selected config", async () => {
    ...
    await withMockedPlatform("linux", () =>
      withEnvAsync(
        {
          INVOCATION_ID: "systemd-invocation",
          GRANTED_SERVICE_MANAGED_ENV_KEYS: "REMOVED_KEY,SECRET_REF_KEY",
  ...
  it("reads gateway password from --password-file", async () => {
    ...
    expect(runtimeErrors).not.toContain(
      "Warning: --password can be exposed via process listings. Prefer --password-file or GRANTED_GATEWAY_PASSWORD.",
    );
  });
