...
type GatewayRunFutureConfigGuardParams = {
  ...
  config?: Pick<GrantedConfig, "env" | "meta"> | null;
};

function resolveGatewayRunFutureConfigBlock(params: GatewayRunFutureConfigGuardParams) {
  const processServiceMode = Boolean(process.env.GRANTED_SERVICE_MARKER?.trim());
  ...
  const candidateServiceMode =
    ...
    Boolean(
      candidateConfig
        ? createConfigRuntimeEnv(candidateConfig, process.env).GRANTED_SERVICE_MARKER?.trim()
        : undefined,
    );
