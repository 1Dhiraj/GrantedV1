...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($GRANTED_CONFIG_PATH, default ~/.openclaw/openclaw.json):",
        ),
      );
      ...
      if (!cfg.discovery?.wideArea?.domain?.trim()) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: set discovery.wideArea.domain in the active OpenClaw config ($GRANTED_CONFIG_PATH, default ~/.openclaw/openclaw.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
