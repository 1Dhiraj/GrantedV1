...
import { jsonSchemaValuesEqual } from "@granted/normalization-core/json-schema";
...
import { getToolPluginMetadata } from "granted/plugin-sdk/tool-plugin";
...
function writeProviderPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  ...
  const indexSource = `import { definePluginEntry } from "granted/plugin-sdk/plugin-entry";
import { createProviderApiKeyAuthMethod } from "granted/plugin-sdk/provider-auth";
...
`;
  const testSource = `import { describe, expect, it } from "vitest";
import type { GrantedPluginApi, ProviderPlugin } from "granted/plugin-sdk/plugin-entry";
import entry from "./index.js";
...
describe(${idLiteral}, () => {
  it("registers the provider and resolves its authenticated catalog", async () => {
    ...
    } as Partial<GrantedPluginApi>;

    entry.register(api as GrantedPluginApi);
