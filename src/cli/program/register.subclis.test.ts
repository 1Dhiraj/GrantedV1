...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.GRANTED_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.GRANTED_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.GRANTED_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.GRANTED_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      ...
      process.env.GRANTED_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });
