...
describe("applyParentDefaultHelpAction (#73077)", () => {
  ...
  beforeEach(() => {
    ...
    originalSuppressHelpBanner = process.env.GRANTED_SUPPRESS_HELP_BANNER;
    process.exitCode = undefined;
  });
  afterEach(() => {
    process.exitCode = originalExitCode;
    if (originalSuppressHelpBanner === undefined) {
      delete process.env.GRANTED_SUPPRESS_HELP_BANNER;
    } else {
      process.env.GRANTED_SUPPRESS_HELP_BANNER = originalSuppressHelpBanner;
    }
  });
  ...
  it("invokes parent help and exits 0 when invoked without subcommand", async () => {
    ...
    const helpSpy = vi.spyOn(parent, "outputHelp").mockImplementation(() => {
      suppressHelpBannerValues.push(process.env.GRANTED_SUPPRESS_HELP_BANNER);
    });
    ...
    expect(process.env.GRANTED_SUPPRESS_HELP_BANNER).toBeUndefined();
