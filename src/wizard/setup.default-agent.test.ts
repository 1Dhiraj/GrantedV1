...
vi.mock("./setup.shared.js", async (importOriginal) => {
  ...
  return {
    ...
    requireRiskAcknowledgement: async ({ config }: { config: GrantedConfig }) => config,
...
vi.mock("./setup.model-auth.js", () => ({
  runSetupModelAuthStep: async ({ config }: { config: GrantedConfig }) => ({
    config,
    authProfiles: [],
...
vi.mock("./setup.gateway-config.js", () => ({
  configureGatewayForSetup: async ({ nextConfig }: { nextConfig: GrantedConfig }) => ({
    nextConfig,
    settings: {
      port: 18789,
      bind: "loopback",
...
describe("runSetupWizard default-agent ownership", () => {
  beforeEach(() => {
    ...
    mocks.writeConfig.mockImplementation(async (nextConfig: GrantedConfig) => nextConfig);
    mocks.setupSkills.mockImplementation(async (nextConfig: GrantedConfig) => nextConfig);
    mocks.setupOfficialPlugins.mockImplementation(
      async ({ config: nextConfig }: { config: GrantedConfig }) => nextConfig,
    );
    mocks.setupRecommendations.mockImplementation(
      async ({ config: nextConfig }: { config: GrantedConfig }) => ({ config: nextConfig }),
    );
