...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers GRANTED_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.GRANTED_GATEWAY_TOKEN;
    process.env.GRANTED_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("keeps GRANTED_GATEWAY_TOKEN in advanced flow when user confirms keeping existing", async () => {
    const prevToken = process.env.GRANTED_GATEWAY_TOKEN;
    process.env.GRANTED_GATEWAY_TOKEN = "advanced-env-token";
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    ...
    process.env.GRANTED_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "GRANTED_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
  ...
  it("routes a seeded quickstart password through the configured SecretRef provider", async () => {
    ...
    const result = await withSecureTestNodeCommand(async (command) =>
      withEnvAsync({ GRANTED_GATEWAY_PASSWORD: undefined }, async () =>
  ...
  it("persists an explicit classic quickstart env token ref", async () => {
    const previous = process.env.GRANTED_GATEWAY_TOKEN;
    process.env.GRANTED_GATEWAY_TOKEN = "token-from-env-ref";
    try {
      const quickstartGateway = resolveQuickstartGatewayDefaults(
        {},
  ...
  it("seeds an explicit env token ref into advanced gateway setup", async () => {
    ...
    try {
      const gatewayDefaults = resolveQuickstartGatewayDefaults(
        {},
        { gatewayPort: 19511, gatewayTokenRefEnv: "GRANTED_GATEWAY_TOKEN" },
      );
      const result = await configureGatewayForSetup({
        flow: "advanced",
        baseConfig: {},
