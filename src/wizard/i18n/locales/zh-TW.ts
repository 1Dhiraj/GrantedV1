...
export const zh_TW = {
  ...
  wizard: {
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 GRANTED_ALLOW_INSECURE_PRIVATE_WS=1。",
    ...
    twitch: {
      ...
      envPrompt: "偵測到 Twitch 環境變數 GRANTED_TWITCH_ACCESS_TOKEN。使用環境 token？",
      helpCopyToken: "3. 複製 token（以 'oauth:' 開頭）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 建立 Twitch 應用",
      helpEnvVars: "支援環境變數：GRANTED_TWITCH_ACCESS_TOKEN",
    ...
    finalize: {
      ...
      gatewayTokenStored:
        "儲存位置：$GRANTED_CONFIG_PATH（預設 ~/.openclaw/openclaw.json）中的 gateway.auth.token，或 GRANTED_GATEWAY_TOKEN。",
