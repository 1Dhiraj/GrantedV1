...
export const zh_CN = {
  ...
  wizard: {
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 GRANTED_ALLOW_INSECURE_PRIVATE_WS=1。",
    ...
    twitch: {
      ...
      envPrompt: "检测到 Twitch 环境变量 GRANTED_TWITCH_ACCESS_TOKEN。使用环境 token？",
      helpCopyToken: "3. 复制 token（以 'oauth:' 开头）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 创建 Twitch 应用",
      helpEnvVars: "支持环境变量：GRANTED_TWITCH_ACCESS_TOKEN",
    ...
    finalize: {
      ...
      gatewayTokenStored:
        "存储位置：$GRANTED_CONFIG_PATH（默认 ~/.openclaw/openclaw.json）中的 gateway.auth.token，或 GRANTED_GATEWAY_TOKEN。",
