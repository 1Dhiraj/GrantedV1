...
describe("shouldBypassAcpDispatchForCommand", () => {
  ...
  it("returns false for plain-text ACP turns", () => {
    ...
    expect(shouldBypassAcpDispatchForCommand(ctx, {} as GrantedConfig)).toBe(false);
  });
  ...
  it.each([
    ...
    expect(shouldBypassAcpDispatchForCommand(ctx, {} as GrantedConfig)).toBe(expected);
  });
  ...
  it("returns true for native ACP slash commands", () => {
    ...
    expect(shouldBypassAcpDispatchForCommand(ctx, {} as GrantedConfig)).toBe(true);
  });
