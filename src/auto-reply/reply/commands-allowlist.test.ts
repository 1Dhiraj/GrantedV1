...
async function withTempConfigPath<T>(
  ...
  const previous = process.env.GRANTED_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      setTestEnvValue("GRANTED_CONFIG_PATH", previous);
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<GrantedConfig>(configPath);
      ...
      createTestRegistry([
        {
          ...
          plugin: {
            ...
            config: {
              ...
              defaultAccountId: (cfg: GrantedConfig) =>
                (cfg.channels?.telegram as TelegramTestSectionConfig | undefined)?.defaultAccount ??
                DEFAULT_ACCOUNT_ID,
            },
          },
