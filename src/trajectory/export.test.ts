...
import type { Message, Usage } from "granted/plugin-sdk/llm";
...
describe("exportTrajectoryBundle", () => {
  ...
  it("uses a pre-rename runtime pointer before current environment overrides", async () => {
    ...
    const previous = process.env.GRANTED_TRAJECTORY_DIR;
    process.env.GRANTED_TRAJECTORY_DIR = envRuntimeDir;
    try {
      const bundle = await exportTrajectoryBundle({
        outputDir,
        sessionFile,
      ...
      if (previous === undefined) {
        ...
        process.env.GRANTED_TRAJECTORY_DIR = previous;
      }
    }
  });
