...
import type { GrantedPluginNodeInvokePolicy } from "../plugins/plugin-registration.types.js";
...
> = {
  cap: string;
  configSchema: GrantedPluginConfigSchema & { parse(value: unknown): Config };
  createNodePolicy(config: Config): GrantedPluginNodeInvokePolicy;
  createRuntime(params: { api: GrantedPluginApi; config: Config }): Runtime;
  description: string;
