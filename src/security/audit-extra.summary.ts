...
function summarizeGroupPolicy(cfg: GrantedConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: GrantedConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: GrantedConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: GrantedConfig): boolean {
...
function isBrowserEnabled(cfg: GrantedConfig): boolean {
...
export function collectAttackSurfaceSummaryFindings(cfg: GrantedConfig): SecurityAuditFinding[] {
