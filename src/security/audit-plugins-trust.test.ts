...
describe("security audit install metadata findings", () => {
  ...
  const runInstallMetadataAudit = async (cfg: GrantedConfig, stateDir: string) => {
  ...
  const writePluginIndexInstallRecords = async (
    ...
    const index: InstalledPluginIndex = {
      ...
      plugins: Object.keys(records).map((pluginId) => ({
        pluginId,
        manifestPath: path.join(stateDir, "extensions", pluginId, "granted.plugin.json"),
    ...
    }> = [
      {
        ...
        run: async () => {
          ...
          writeHookInstalls(stateDir, {
            "test-hooks": { source: "npm", spec: "@granted/test-hooks" },
          });
        ...
        run: async () => {
          ...
          await writePluginIndexInstallRecords(stateDir, {
            "voice-call": {
              source: "npm",
              spec: "@granted/voice-call",
              resolvedSpec: "@granted/voice-call@1.2.3",
              integrity: "sha512-plugin",
            },
          });
          writeHookInstalls(stateDir, {
            "test-hooks": {
              ...
              resolvedSpec: "@granted/test-hooks@1.2.3",
              integrity: "sha512-hook",
            },
          });
          return runInstallMetadataAudit({}, stateDir);
...
describe("security audit extension tool reachability findings", () => {
  ...
  const runSharedExtensionsAudit = async (config: GrantedConfig) => {
