...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: GrantedConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: GrantedConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: GrantedConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): GrantedConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: GrantedConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
