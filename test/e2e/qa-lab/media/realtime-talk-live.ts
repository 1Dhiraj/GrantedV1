...
function createRealtimeTalkEvidenceWriter(
  ...
  const model = env.GRANTED_REALTIME_OPENAI_MODEL?.trim() || DEFAULT_OPENAI_MODEL;
...
async function runRealtimeTalkLiveProof(params: {
  ...
  const model = params.env.GRANTED_REALTIME_OPENAI_MODEL?.trim() || DEFAULT_OPENAI_MODEL;
  const voice = params.env.GRANTED_REALTIME_OPENAI_VOICE?.trim() || DEFAULT_OPENAI_VOICE;
