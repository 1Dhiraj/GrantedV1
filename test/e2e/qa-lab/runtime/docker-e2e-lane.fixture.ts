...
const QA_DOCKER_E2E_LANES = {
  ...
  "update-migration": {
    env: (env) => ({
      GRANTED_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.GRANTED_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "openclaw@latest",
      GRANTED_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
      GRANTED_UPGRADE_SURVIVOR_SCENARIO:
        env.GRANTED_UPGRADE_SURVIVOR_SCENARIO ?? "plugin-deps-cleanup",
    }),
  ...
  "update-restart-auth": {
    env: (env) => ({
      GRANTED_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.GRANTED_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "openclaw@latest",
      GRANTED_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT:
        env.GRANTED_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT ?? "1500s",
      GRANTED_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
