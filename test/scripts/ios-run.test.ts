...
function makeFixture(bundleId: string): { root: string; script: string; logFile: string } {
  ...
  writeExecutable(
    ...
    `#!/usr/bin/env bash
set -euo pipefail
if [[ -n "\${GRANTED_SIMULATOR_PUSH_PROOF_SECRET:-}" || -n "\${CUSTOM_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
if (process.env.GRANTED_SIMULATOR_PUSH_PROOF_SECRET || process.env.CUSTOM_SIMULATOR_PUSH_PROOF_SECRET) {
...
set -euo pipefail
printf 'xcodegen %s\\n' "$*" >>"${logFile}"
if [[ -n "\${GRANTED_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
printf 'simctl %s\\n' "$*" >>"${logFile}"
if [[ "$1" == "launch" ]]; then
  if [[ -n "\${SIMCTL_CHILD_GRANTED_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
describe("scripts/ios-run.sh", () => {
  ...
  it("builds simulator sandbox relay mode and injects proof secret only at launch", () => {
    ...
    runIosRun(
      fixture,
      {
        GRANTED_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
    ...
    expect(log).toContain("GRANTED_PUSH_MODE=simulatorSandbox");
    expect(log).toContain("GRANTED_PUSH_RELAY_BASE_URL=https://ios-push-relay-sandbox.openclaw.ai");
  ...
  it("scrubs exported simulator proof secrets from normal build helpers", () => {
    ...
    runIosRun(fixture, {
      GRANTED_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
      GRANTED_SIMULATOR_PUSH_PROOF_SECRET_ENV: "CUSTOM_SIMULATOR_PUSH_PROOF_SECRET",
