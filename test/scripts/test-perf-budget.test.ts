...
describe("test perf budget script", () => {
  it("requires a budget source unless report-only mode is explicit", () => {
    ...
    expect(
      testing.parseArgs([], {
        GRANTED_TEST_PERF_REPORT_ONLY: "yes",
      }),
    ).toMatchObject({
      baselineWallMs: null,
      maxWallMs: null,
  ...
  it("parses numeric budget env vars strictly before running Vitest", () => {
    expect(
      testing.parseArgs([], {
        GRANTED_TEST_PERF_BASELINE_WALL_MS: "1000",
        GRANTED_TEST_PERF_MAX_REGRESSION_PCT: "12.5",
        GRANTED_TEST_PERF_MAX_WALL_MS: "1500",
      }),
    ).toMatchObject({
      baselineWallMs: 1000,
    ...
    expect(() =>
      testing.parseArgs([], {
        GRANTED_TEST_PERF_MAX_WALL_MS: "1000ms",
      }),
    ).toThrow("GRANTED_TEST_PERF_MAX_WALL_MS must be a non-negative number");
  });
