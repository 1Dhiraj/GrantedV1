...
`,
  ...
  const result = spawnSync("bash", [`scripts/docker/install-sh-${runner}/run.sh`], {
    ...
    env: {
      ...
      GRANTED_E2E_MODELS: "openai",
      GRANTED_INSTALL_TAG: options.target,
      GRANTED_INSTALL_E2E_PREVIOUS: options.previous ?? "",
      GRANTED_INSTALL_SMOKE_PREVIOUS: options.previous ?? "",
      GRANTED_INSTALL_E2E_SKIP_PREVIOUS: options.skipPrevious ? "1" : "0",
      GRANTED_INSTALL_SMOKE_SKIP_PREVIOUS: options.skipPrevious ? "1" : "0",
      GRANTED_INSTALL_SMOKE_HEARTBEAT_INTERVAL: "0",
    },
  });
