...
function createStatusHarness(mac: MacScriptFixture, permissionMode: "fail" | "invalid") {
  ...
  writeCommandFixture(
    ...
    [
      ...
      "  GrantedBuildTimestamp) printf '%s\\n' '2026-08-28T00:00:00Z' ;;",
      "  OpenClawWorkerBuildID) printf '%s\\n' 'fixture-build' ;;",
      '  ProgramArguments) printf \'["%s/Contents/MacOS/OpenClaw","--elevation-host"]\\n\' "$TEST_APP_PATH" ;;',
      "  EnvironmentVariables.GRANTED_STATE_DIR) printf '%s\\n' \"$TEST_STATE_DIR\" ;;",
      "  EnvironmentVariables.GRANTED_CONFIG_PATH) printf '%s\\n' \"$TEST_CONFIG_PATH\" ;;",
...
function createMigrationPlanHarness(
  ...
  writeFileSync(
    ...
    [
      ...
      `<key>GRANTED_STATE_DIR</key><string>${stateDir}</string>`,
      `<key>GRANTED_CONFIG_PATH</key><string>${configPath}</string>`,
      "</dict>",
      "</dict></plist>",
      "",
    ].join("\n"),
...
function createCanonicalNodeMigrationHarness(mac: MacScriptFixture, nodeId = "fixture-node") {
  ...
  writeFileSync(
    ...
    [
      ...
      `export GRANTED_STATE_DIR='${harness.stateDir}'`,
      `export GRANTED_CONFIG_PATH='${harness.configPath}'`,
      "export GRANTED_GATEWAY_TOKEN='ignored-secret-shape'",
      "",
    ].join("\n"),
    "utf8",
  );
...
function createArtifactVerificationHarness(mac: MacScriptFixture) {
  ...
  writeCommandFixture(
    ...
    [
      ...
      'plutil -insert GrantedBuildTimestamp -string 2026-08-28T00:00:00Z "$app/Contents/Info.plist"',
...
describe("mac elevation host command contract", () => {
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        const plist = readFileSync(harness.plistPath, "utf8").replace(
          `<key>GRANTED_CONFIG_PATH</key><string>${harness.configPath}</string>\n`,
          "",
        );
        ...
        expect(result.stderr).toContain(
          "--config-path does not match the migration LaunchAgent GRANTED_CONFIG_PATH",
        );
      }),
  );
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        writeFileSync(
          ...
          [
            "# Generated by OpenClaw. Do not edit while the gateway service is installed.",
            "export GRANTED_STATE_DIR='relative-state'",
            `export GRANTED_CONFIG_PATH='${harness.configPath}'`,
            "",
          ].join("\n"),
          "utf8",
        ...
        expect(result.status).toBe(1);
        expect(result.stderr).toContain("canonical node GRANTED_STATE_DIR must be absolute");
      }),
  );
  ...
  it.skipIf(process.platform !== "darwin").concurrent(
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        writeFileSync(
          harness.envPath,
          `${readFileSync(harness.envPath, "utf8")}GRANTED_STATE_DIR=/attacker-selected\n`,
          "utf8",
        );
        const result = await runInstaller(
          mac,
  ...
  it.concurrent("relaunches an adopted app only after exact exit with its selected state and config", async ({
    ...
    mac.lifetime.run(async () => {
      ...
      expect(relaunchBody).toContain('--env "GRANTED_STATE_DIR=$STATE_DIR"');
      expect(relaunchBody).toContain('--env "GRANTED_CONFIG_PATH=$CONFIG_PATH"');
  ...
  it.concurrent("builds an immutable source-addressed notarized ZIP with a portable installer", async ({
    ...
    mac.lifetime.run(async () => {
      ...
      expect(script).toContain("GRANTED_MAC_SIGNING_VARIANT=elevation-host");
