...
fs.appendFileSync(env.FAKE_GH_CALLS, JSON.stringify({ route, argv, override: env.GRANTED_GH_BIN ?? null }) + "\\n");
if (argv[0] === "auth" && argv[1] === "token") {
  ...
  const result = JSON.stringify({ route, argv, colors, tokens, override: env.GRANTED_GH_BIN ?? null, cwd: process.cwd() });
...
describe.each(engines)("%s plain gh execution", (engine) => {
  it.each([undefined, ""])(
    ...
    (override) => {
      ...
      fixture.env.GRANTED_GH_BIN = override;
  ...
  it.each(["missing PATH command", "nonexecutable override"])(
    ...
    (failure) => {
      ...
      if (failure === "missing PATH command") {
        ...
        fixture.env.GRANTED_GH_BIN = path.join(fixture.root, "not-executable");
        writeFileSync(fixture.env.GRANTED_GH_BIN, "not executable");
  ...
  it.each(tokenNames)("preserves ambient %s without probing in either route", (tokenName) => {
    ...
    for (const explicit of [false, true]) {
      fixture.env.GRANTED_GH_BIN = explicit ? fixture.override : undefined;
      const before = { ...fixture.env };
      const result = JSON.parse(runGh(engine, fixture.env));
      expect(result).toMatchObject({
  ...
  it.each([
    ...
    ({ host, tokenName, args }) => {
      const fixture = makeFixture();
      Object.assign(fixture.env, { GH_HOST: host, GRANTED_GH_BIN: fixture.override });
      const before = { ...fixture.env };
      const output =
        engine === "Node"
          ? execFileSync(
...
describe("plain gh subprocess contracts", () => {
  ...
  it("keeps explicit reads override-independent and parses normalized JSON", async () => {
    const fixture = makeFixture();
    fixture.env.GRANTED_GH_BIN = "/invalid-explicit-override";
    ...
    expect(fixture.env.GRANTED_GH_BIN).toBe("/invalid-explicit-override");
  });

  it("preserves asynchronous read refusals and buffer limits without another route", async () => {
    const fixture = makeFixture();
    fixture.env.GRANTED_GH_BIN = fixture.override;
    fixture.env.FAKE_GH_REJECT = "1";
