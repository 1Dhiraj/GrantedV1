...
function makeEnv(overrides: Record<string, string | undefined> = {}) {
  ...
  if (!Object.hasOwn(overrides, "GRANTED_LOCAL_CHECK_MODE")) {
    delete env.GRANTED_LOCAL_CHECK_MODE;
  }
...
describe("local-check-runtime", () => {
  ...
  it("reenables local check policy for local wrapper entrypoints", () => {
    expect(resolveLocalCheckEnv({ GRANTED_LOCAL_CHECK: "0", PATH: "/usr/bin" })).toEqual({
      GRANTED_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
    expect(resolveLocalCheckEnv({ GRANTED_LOCAL_CHECK: "false", PATH: "/usr/bin" })).toEqual({
      GRANTED_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
  });
  ...
  it("skips declaration transforms for no-emit tsgo checks", () => {
    const { args } = applyLocalTsgoPolicy([], makeEnv({ GRANTED_LOCAL_CHECK: "0" }), ROOMY_HOST);
  ...
  it("keeps explicit tsgo flags and Go env overrides intact when throttled", () => {
    const { args, env } = applyLocalTsgoPolicy(
      ...
      makeEnv({
        ...
        GOGC: "80",
        GOMEMLIMIT: "5GiB",
        GRANTED_TSGO_PPROF_DIR: "/tmp/profile",
      }),
      CONSTRAINED_HOST,
    );
  ...
  it("keeps explicit tsgo declaration flags intact", () => {
    const env = makeEnv({ GRANTED_LOCAL_CHECK_MODE: "full" });
  ...
  it("uses the configured local tsgo build info file", () => {
    const { args } = applyLocalTsgoPolicy(
      [],
      makeEnv({
        GRANTED_LOCAL_CHECK_MODE: "full",
        GRANTED_TSGO_BUILD_INFO_FILE: ".artifacts/custom/tsgo.tsbuildinfo",
      }),
      ROOMY_HOST,
    );
  ...
  it("avoids incremental cache reuse for ad hoc tsgo runs", () => {
    const { args } = applyLocalTsgoPolicy(
      ["--extendedDiagnostics"],
      makeEnv({ GRANTED_LOCAL_CHECK_MODE: "full" }),
      ROOMY_HOST,
    );
  ...
  it("does not oversubscribe a single-CPU host", () => {
    const { env } = applyLocalTsgoPolicy([], makeEnv({ GRANTED_LOCAL_CHECK_MODE: "throttled" }), {
      logicalCpuCount: 1,
...
`;
      ...
      const env = makeEnv({
        ...
        GRANTED_OXLINT_SKIP_PREPARE: undefined,
        ...goEnv,
      });
