...
const resources = ['HOME', 'CFFIXED_USER_HOME', 'TMPDIR', 'GRANTED_STATE_DIR', 'GRANTED_CONFIG_PATH'];
...
function awaitSignal(ownedKeychain) {
  process.on('SIGTERM', () => {
    fs.appendFileSync(${JSON.stringify(log)}, JSON.stringify({tool: 'shutdown', resourcesPresent: fs.existsSync(env.HOME) && fs.existsSync(env.GRANTED_STATE_DIR), keychainPresent: !!ownedKeychain && fs.existsSync(ownedKeychain)}) + '\\n');
    process.exit(0);
  });
...
if (tool === 'swift' && args[0] === 'test') {
  if (env.GRANTED_STATE_DIR !== ${JSON.stringify(path.join(root, "ambient-state"))}) {
    fs.writeFileSync(path.join(env.GRANTED_STATE_DIR, 'child-owned'), 'fixture');
  }
  if (${JSON.stringify(waitForSignal)} === 'swift') awaitSignal(settings.default);
  else process.exit(env.GRANTED_PROFILE === 'default' ? ${defaultExitCode} : ${namedExitCode});
}
`;
  ...
  const env = {
    ...
    GRANTED_PROFILE: "ambient-fixture",
    GRANTED_STATE_DIR: path.join(root, "ambient-state"),
    GRANTED_CONFIG_PATH: path.join(root, "ambient-config.json"),
    GRANTED_GATEWAY_TOKEN: "synthetic-not-a-credential",
...
describe.skipIf(process.platform === "win32")("native test launch ownership", () => {
  ...
  it.each([
    ...
    ({ defaultCode, namedCode, logicalCpu, expectedWidth }) => {
      ...
      for (const [index, test] of tests.entries()) {
        ...
        if (index === 0) {
          expect(test.env.GRANTED_PROFILE).toBe("default");
        } else {
          expect(test.env.GRANTED_PROFILE).toMatch(/^test-[a-z0-9-]+$/);
        }
        expect(test.env.GRANTED_PROFILE).not.toBe(f.env.GRANTED_PROFILE);
        expect(test.env.GRANTED_GATEWAY_TOKEN).toBeUndefined();
        for (const key of [
          "DEVELOPER_DIR",
          "DYLD_FRAMEWORK_PATH",
          "DYLD_LIBRARY_PATH",
        ...
        for (const key of ["HOME", "TMPDIR", "GRANTED_STATE_DIR", "GRANTED_CONFIG_PATH"]) {
          expect(test.env[key].startsWith(`${ownedRoot}/`)).toBe(true);
          expect(test.present[key]).toBe(key !== "GRANTED_CONFIG_PATH");
        }
  ...
  it("uses fresh named preferences for successive launches", () => {
    ...
    expect(calls[0].env.GRANTED_PROFILE).not.toBe(calls[1].env.GRANTED_PROFILE);
