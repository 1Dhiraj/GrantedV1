  ...
  it("flags wildcard re-exports from plugin-sdk subpaths", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export * from "granted/plugin-sdk/foo";',
          'export * as sdk from "granted/plugin-sdk/foo";',
          'export type * from "granted/plugin-sdk/bar";',
          'export type * as sdkTypes from "granted/plugin-sdk/bar";',
          'export { named } from "granted/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "granted/plugin-sdk/foo";' },
      { line: 2, text: 'export * as sdk from "granted/plugin-sdk/foo";' },
      { line: 3, text: 'export type * from "granted/plugin-sdk/bar";' },
      { line: 4, text: 'export type * as sdkTypes from "granted/plugin-sdk/bar";' },
    ]);
  });
  ...
  it("allows explicit SDK exports and local wildcard barrels", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export { named } from "granted/plugin-sdk/foo";',
          'export type { Named } from "granted/plugin-sdk/foo";',
    ...
    expect(JSON.parse(result.stdout)).toEqual([
      {
        ...
        line: 1,
        text: 'export * from "granted/plugin-sdk/foo";',
      },
    ]);
  });
});
