...
describe("stable release closeout", () => {
  ...
  it("requires complete platform assets for failed-publish recovery", () => {
    ...
    expect(missing.errors).toContain(
      "GitHub release v2026.6.8 Windows asset names do not match the recovery contract: expected GrantedCompanion-SHA256SUMS.txt, GrantedCompanion-Setup-arm64.exe, GrantedCompanion-Setup-x64.exe; got <none>.",
    );
    expect(missing.errors).toContain(
    ...
    const complete = verifyStableMainCloseout({
      ...
      windowsNodeInstallerDigests: {
        "GrantedCompanion-Setup-arm64.exe": `sha256:${"1".repeat(64)}`,
        "GrantedCompanion-Setup-x64.exe": `sha256:${"2".repeat(64)}`,
      },
      release: {
        ...release,
        assets: [
          ...
          {
            name: "GrantedCompanion-SHA256SUMS.txt",
