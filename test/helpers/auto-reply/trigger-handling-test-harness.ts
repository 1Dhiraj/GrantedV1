...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    openclawHome: process.env.GRANTED_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("GRANTED_HOME", snapshot.openclawHome);
  restoreKey("GRANTED_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.GRANTED_STATE_DIR = join(home, ".openclaw");
...
export function makeCfg(home: string): GrantedConfig {
