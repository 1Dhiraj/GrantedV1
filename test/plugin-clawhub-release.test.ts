...
describe("collectClawHubPublishablePluginPackages", () => {
  it("rejects duplicate ClawHub package names from different plugin directories", () => {
    ...
    expect(() => collectClawHubPublishablePluginPackages(repoDir)).toThrow(
      "package @granted/demo-plugin is declared by multiple plugin sources: demo-plugin (extensions/demo-plugin), demo-shadow (extensions/demo-shadow).",
    );
  });
  ...
  it("validates only selected package names when filters are provided", () => {
    ...
    expect(
      collectClawHubPublishablePluginPackages(repoDir, {
        packageNames: ["@granted/demo-plugin"],
      }).map((plugin) => plugin.packageName),
    ).toEqual(["@granted/demo-plugin"]);
  });
...
describe("OpenClaw dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "cohere",
      packageName: "@granted/cohere-provider",
      install: {
    ...
    },
    {
      ...
      packageName: "@granted/diagnostics-prometheus",
      install: {
        clawhubSpec: "clawhub:@granted/diagnostics-prometheus",
        defaultChoice: "npm",
        minHostVersion: ">=2026.4.25",
        npmSpec: "@granted/diagnostics-prometheus",
      },
    },
    {
      extensionId: "gmi",
      packageName: "@granted/gmi-provider",
      install: {
    ...
    {
      extensionId: "novita",
      packageName: "@granted/novita-provider",
      install: {
        clawhubSpec: "clawhub:@granted/novita-provider",
        defaultChoice: "npm",
        minHostVersion: ">=2026.7.2",
        npmSpec: "@granted/novita-provider",
      },
    },
  ] as const;
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@granted/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
...
describe("collectPluginClawHubReleasePlan", () => {
  it("bounds parallel ClawHub package-state reads and preserves plan order", async () => {
    ...
    const packageNames = ["demo-plugin", ...extraExtensionIds].map(
      (extensionId) => `@granted/${extensionId}`,
    );
  ...
  it("rejects stale required dependencies before querying ClawHub", async () => {
    ...
    await expect(
      ...
      '@granted/demo-plugin@2026.4.1: demo-runtime must match npm latest for release; found "1.2.3", latest is "1.2.4".',
    );
  });
  ...
  it("accepts required dependencies matching npm latest", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@granted/demo-plugin@2026.4.1": 404,
      },
    });
    ...
    expect(plan.candidates.map((plugin) => plugin.packageName)).toEqual(["@granted/demo-plugin"]);
  });
  ...
  it("fails closed when npm latest cannot be resolved", async () => {
    ...
    await expect(
      ...
      "@granted/demo-plugin@2026.4.1: could not resolve npm latest for demo-runtime: registry unavailable",
    );
  });
  ...
  it("preserves ClawHub response details after package retries are exhausted", async () => {
    ...
    await expect(
      ...
      "Failed to query ClawHub package @granted/demo-plugin: 503 Rate limit temporarily unavailable [retry-after=1; x-request-id=request-123]",
    );
  ...
  it.each([
    ...
    async ({ responseBody, expectedDetail }) => {
      ...
      await expect(
        ...
        `Failed to query ClawHub package @granted/demo-plugin: 503 ${expectedDetail}`,
      );
    },
  );
  ...
  it("skips versions that already exist on ClawHub", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@granted/demo-plugin@2026.4.1": 200,
      },
    });
...
describe("buildOpenClawReleaseClawHubPlan", () => {
  it("emits a dispatch plan that keeps release bytes separate from protected tooling", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@granted/demo-plugin@2026.4.1": 404,
        "@granted/demo-three@2026.4.1": 404,
      },
    });
    ...
    expect(plan.normal).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@granted/demo-plugin"],
      inputs: {
        publish_scope: "selected",
        ref: "a".repeat(40),
        release_tag: "v2026.4.1-beta.1",
        plugins: "@granted/demo-plugin",
    ...
    expect(plan.bootstrap).toEqual({
      ...
      ref: `release-publish/${"d".repeat(12)}-12345`,
      shouldDispatch: true,
      packages: ["@granted/demo-two", "@granted/demo-three"],
      inputs: {
        bootstrap_workflow_sha: "d".repeat(40),
        ref: "a".repeat(40),
        release_tag: "v2026.4.1-beta.1",
        plugins: "@granted/demo-two,@granted/demo-three",
        release_publish_run_attempt: "2",
        release_publish_run_id: "12345",
        release_publish_branch: "main",
      },
    ...
    expect(plan.summary).toEqual({
      ...
      normalPlugins: "@granted/demo-plugin",
      bootstrapPlugins: "@granted/demo-two,@granted/demo-three",
      missingTrustedPlugins: "@granted/demo-three",
    });
  ...
  it("routes already-published packages missing trusted publisher config to bootstrap repair", async () => {
    ...
    expect(plan.summary).toMatchObject({
      ...
      bootstrapCount: 1,
      missingTrustedPublisherCount: 1,
      bootstrapPlugins: "@granted/demo-plugin",
      missingTrustedPlugins: "@granted/demo-plugin",
    });
  });
...
describe("runPluginClawHubReleaseCheck", () => {
  it("rejects stale required dependencies", async () => {
    ...
    await expect(
      runPluginClawHubReleaseCheck(["--plugins", "@granted/demo-plugin"], {
        rootDir: repoDir,
        resolveLatestVersion: () => "1.2.4",
      }),
    ).rejects.toThrow(
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@granted/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
...
`,
  ...
  it("rejects duplicate normalized paths before invoking the ClawHub CLI", () => {
    ...
    const tgzBytes = createClawPackBytes("@granted/demo-plugin", "2026.4.1", {
      ...
      execFileSync(
        ...
        [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--validate-packed", tgzPath],
        {
          ...
          env: {
            ...
            EXPECTED_CLAWHUB_PACKAGE_NAME: "@granted/demo-plugin",
...
if [[ " $* " == *" --dry-run "* ]]; then
  printf '{"name":"@granted/demo-plugin","version":"2026.4.1"}\\n'
  exit 0
fi
attempts=0
