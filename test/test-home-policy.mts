...
export function resolveTestHomePolicy(
  ...
  const allowRealHome = !hermetic && isTruthyTestEnvValue(env.GRANTED_LIVE_USE_REAL_HOME);
  return {
    hermetic,
    live,
    allowRealHome,
  ...
  if (policy.live && policy.allowRealHome && mode !== "live-aware") {
    throw new Error(
      "[vitest] explicit real-home live execution requires a known wholly live-aware selection; " +
        `the selection is ${mode}. Run hermetic tests without LIVE, GRANTED_LIVE_TEST, ` +
        "GRANTED_LIVE_GATEWAY and GRANTED_LIVE_USE_REAL_HOME " +
