...
describe("granted launcher", () => {
  ...
  it("prints recovery guidance before legacy-incompatible modules can load", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", pathToFileURL(legacyRuntimePath).href, path.join(fixtureRoot, "granted.mjs")],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("rejects Bun without node:sqlite even when its Node compatibility version is new enough", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", pathToFileURL(mockRuntime).href, path.join(fixtureRoot, "granted.mjs")],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("executes an entry.mjs-only compiled entry through the root launcher", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", import.meta.resolve("tsx"), path.join(fixtureRoot, "granted.mjs"), "--profile"],
      {
        cwd: process.cwd(),
  ...
  it.runIf(process.env.GRANTED_TEST_BUN_LAUNCHER === "1" && hasBunRuntime())(
    ...
    async () => {
      ...
      const result = spawnSync(bunBin, [path.join(fixtureRoot, "granted.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
      });
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "granted.mjs"), params.command, "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each(["config", "doctor", "gateway", "models", "plugins", "sessions", "tasks"])(
    ...
    async (command) => {
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "granted.mjs"), command, "--help"],
        {
          cwd: fixtureRoot,
          env: launcherEnv(),
          encoding: "utf8",
  ...
  it("uses precomputed subcommand help with leading root options", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "granted.mjs"), "--log-level", "warn", "--no-color", "models", "-h"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each(
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "granted.mjs"), ...args], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
  ...
  it("defers precomputed subcommand help to the runtime entry when container env is set", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "granted.mjs"), "models", "--help"],
      {
        cwd: fixtureRoot,
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "granted.mjs"), ...params.args],
      {
        cwd: fixtureRoot,
  ...
  it("defers nodes help to the runtime entry when plugin config can change help", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "granted.mjs"), "nodes", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ GRANTED_CONFIG_PATH: configPath }),
        encoding: "utf8",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const launcher = spawn(process.execPath, [path.join(fixtureRoot, "granted.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv({
          NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
        }),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "granted.mjs"), "hooks", "relay"],
        {
          cwd: fixtureRoot,
          env: launcherEnv({
            NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
