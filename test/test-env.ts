...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.GRANTED_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    ...
    { key: "GRANTED_TEST_FAST", value: process.env.GRANTED_TEST_FAST },
    {
      key: "GRANTED_STRICT_FAST_REPLY_CONFIG",
      value: process.env.GRANTED_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "GRANTED_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.GRANTED_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "GRANTED_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.GRANTED_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "GRANTED_STATE_DIR", value: process.env.GRANTED_STATE_DIR },
    { key: "GRANTED_CONFIG_PATH", value: process.env.GRANTED_CONFIG_PATH },
    { key: "GRANTED_GATEWAY_PORT", value: process.env.GRANTED_GATEWAY_PORT },
    { key: "GRANTED_BRIDGE_ENABLED", value: process.env.GRANTED_BRIDGE_ENABLED },
    { key: "GRANTED_BRIDGE_HOST", value: process.env.GRANTED_BRIDGE_HOST },
    { key: "GRANTED_BRIDGE_PORT", value: process.env.GRANTED_BRIDGE_PORT },
    { key: "GRANTED_CANVAS_HOST_PORT", value: process.env.GRANTED_CANVAS_HOST_PORT },
    { key: "GRANTED_TEST_HOME", value: process.env.GRANTED_TEST_HOME },
    { key: "GRANTED_AGENT_DIR", value: process.env.GRANTED_AGENT_DIR },
...
function initializeIsolatedTestEnv(tempHome: string): void {
  ...
  setTestEnvValue("GRANTED_TEST_HOME", tempHome);
  setTestEnvValue("GRANTED_TEST_FAST", "1");
  setTestEnvValue("GRANTED_STRICT_FAST_REPLY_CONFIG", "1");
  deleteTestEnvValue("GRANTED_ALLOW_SLOW_REPLY_TESTS");

  // GRANTED_HOME takes precedence over HOME, so both must be isolated together.
  ...
  deleteTestEnvValue("GRANTED_BRIDGE_ENABLED");
  deleteTestEnvValue("GRANTED_BRIDGE_HOST");
  deleteTestEnvValue("GRANTED_BRIDGE_PORT");
  deleteTestEnvValue("GRANTED_CANVAS_HOST_PORT");
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.GRANTED_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const realConfigPath = params.env.GRANTED_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.GRANTED_CONFIG_PATH, realOpenClawHome)
