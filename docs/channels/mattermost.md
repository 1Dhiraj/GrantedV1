...
<Steps>
  <Step title="Ensure plugin is available">
    Install `@granted/mattermost` with the command above, then restart the Gateway if it is already running.
  </Step>
...
External scripts and webhooks can post buttons directly via the Mattermost REST API instead of going through the agent's `message` tool. Prefer OpenClaw's `message` tool. For direct integrations, import `buildButtonAttachments` from `@granted/mattermost/api.js`; if posting raw JSON, follow these rules:
